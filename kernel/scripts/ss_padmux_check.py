    ...
    for check_num in range(len(pad_digit)):
        ...
        while compared_num < (len(pad_digit)):
            if pad_digit[check_num] != pad_digit[compared_num]:
                ...
                print('\033[0;31;40m[padmux setting error]\033[0m Please confirm whether the PAD with the \033[0;31;40mPAD INDEX:', pad_digit[check_num], '\033[0m is reused !')
                exit(-1)
    ...
    for check_num in range(len(mdrv_puse)):
        ...
        while compared_num < (len(mdrv_puse)):
            if mdrv_puse[check_num] != mdrv_puse[compared_num]:
                ...
                print('\033[0;31;40m[padmux setting error]\033[0m Please confirm whether the PUSE with the \033[0;31;40mPAD INDEX:', pad_digit[check_num], '\033[0m is reused !')
                exit(-1)
        ...
        for pin_compared_num in range(len(pinmux_name)):
            if pad_mode[check_num] == pinmux_mode[pin_compared_num]:
                if pinmux_name[pin_compared_num] not in pad_digit:
                    raise Exception('[\033[0;31;40mpadmux setting error\033[0m] Please write all the Pads corresponding to one ModeVal! the gpio index is :', pad_digit[check_num])
                    exit(-1)
                else:
