...
/*
+------------------------------------------------------------------------------
| FUNCTION    : msb250x_ep0_clr_opr_set_ss
+------------------------------------------------------------------------------
| DESCRIPTION : to set the SendStall & ServiceRxPktRdy bit to stall EP0
|
| RETURN      : NULL
+------------------------------------------------------------------------------
| Variable Name      |IN |OUT|                   Usage
...
{
    ms_writeb((MSB250X_OTG0_CSR0_SRXPKTRDY | MSB250X_OTG0_CSR0_SENDSTALL), MSB250X_OTG0_EP0_CSR0_REG);
}
EXPORT_SYMBOL(msb250x_ep0_clr_opr_set_ss);
                      ...
                      const struct usb_endpoint_descriptor *desc)
{
    ...
    _ep->mult = usb_endpoint_maxp_mult(desc);
...
int msb250x_ep_disable(struct usb_ep *_ep)
{
    ...
    printk(KERN_DEBUG"<USB>[EP][%d] disabled\n", ep_num);
//printb();
    return 0;
}
                       ...
                       struct usb_request *_req)
{
    ...
    {
#if 0
        printk(KERN_DEBUG"dequeued req %p from %s, len %d buf %p\n",
...
{
    u8 testPacket[53] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xAA,
    ...
    case TEST_J:
        printk("TEST_J mode\n");
        ms_writeb(MSB250X_OTG0_TESTMODE_TEST_J, MSB250X_OTG0_TESTMODE_REG);
        break;
    case TEST_K:
        printk("TEST_K mode\n");
        ms_writeb(MSB250X_OTG0_TESTMODE_TEST_K, MSB250X_OTG0_TESTMODE_REG);
        break;
    case TEST_SE0_NAK:
        printk("TEST_SE0_NAK mode\n");
        ms_writeb(MSB250X_OTG0_TESTMODE_TEST_SE0_NAK, MSB250X_OTG0_TESTMODE_REG);
        break;
    case TEST_PACKET:
        printk("TEST_PACKET mode\n");
        ms_writeb(MSB250X_OTG0_TESTMODE_TEST_PACKET, MSB250X_OTG0_TESTMODE_REG);
        ms_writesb(testPacket, OTG0_EP_FIFO_ACCESS_L(0), 53);
    ...
    if (dev->req_std)
    {
        switch (ctrl_req.bRequest)
        {
            ...
            case USB_REQ_CLEAR_FEATURE:
                ...
                if(ctrl_req.bRequestType == USB_RECIP_DEVICE)
                {
                    if(ctrl_req.wValue==USB_DEVICE_REMOTE_WAKEUP)//USB_DEVICE_REMOTE_WAKEUP
                    {
                        dev->devstatus &= ~(1 << USB_DEVICE_REMOTE_WAKEUP);
                        printk(KERN_DEBUG "USB_REQ_CLEAR_FEATURE USB_DEVICE_REMOTE_WAKEUP: %d\n", dev->devstatus);
                        msb250x_ep0_clear_opr();
                        return;
                    }
                }
            ...
            case USB_REQ_SET_FEATURE:
                ...
                if(ctrl_req.bRequestType == USB_RECIP_DEVICE)
                {
                    if(ctrl_req.wValue == USB_DEVICE_TEST_MODE) //USB20_TEST_MODE
                    {
