                        ...
                        struct usb_ss_ep_comp_descriptor *ep_comp)
{
    ...
    {
        if ((maxpacket > dev->ep[index].ep.maxpacket_limit) || (maxpacket * mult > dev->ep[index].fifo_size) || dev->ep[index].ep.claimed)
            continue;
    ...
    if ((maxpacket <= dev->ep[select].ep.maxpacket_limit) && (maxpacket * mult <= dev->ep[select].fifo_size) && !dev->ep[select].ep.claimed)
    {
        ep = &dev->ep[select].ep;
...
int msb250x_gadget_wakeup(struct usb_gadget *g)
{
    ...
    if (dev->devstatus & (1 << USB_DEVICE_REMOTE_WAKEUP))
    {
        ...
        ms_writeb(power, MSB250X_OTG0_PWR_REG);
        printk(KERN_INFO "MSB250X issue wakeup\n");
