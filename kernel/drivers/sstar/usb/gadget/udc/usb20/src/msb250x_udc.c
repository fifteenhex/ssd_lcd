...
static uint tx_swing = 115;
module_param(tx_swing, uint, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(tx_swing, "main current");

static uint dem_cur = 105;
module_param(dem_cur, uint, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(dem_cur, "de-emphasis current");

static uint cm_cur = 110;
module_param(cm_cur, uint, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(cm_cur, "cm current");
    ...
    if (1 == usb_st->bSuspend)
    {
        ...
        if (1 == pst_power->bSuspendMode)
        {
            if (dev->gadget.speed != USB_SPEED_UNKNOWN &&
                ...
                dev->driver->suspend)
            {
                printk(KERN_INFO "call gadget->suspend\n");
#if defined(CONFIG_PM) && defined(USB_SUPPORT_WAKEUP_XTAL)
                /* reset usb irq status for xtal wake up */
                ms_writeb(ms_readb(GET_REG8_ADDR(USBC_BASE_ADDR, 0xb)) & (~(0x1 << 6)), GET_REG8_ADDR(USBC_BASE_ADDR, 0xb));
                ms_writeb(ms_readb(GET_REG8_ADDR(USBC_BASE_ADDR, 0xb)) | (0x1 << 6), GET_REG8_ADDR(USBC_BASE_ADDR, 0xb));
#endif
                spin_unlock(&dev->lock);
...
static void msb250x_udc_reinit(struct msb250x_udc *dev)
{
    ...
    for (ep_num = 0; ep_num < MSB250X_MAX_ENDPOINTS; ep_num++)
    {
        ...
        usb_ep_set_maxpacket_limit(&ep->ep, dev->ep[ep_num].ep.maxpacket_limit);
...
void msb250x_udc_init_usb_ctrl(void)
{
    ...
    /* USB Resume IRQ Enable and RESET IRQ Enable */
    eve_en_l = ms_readb(GET_REG16_ADDR(USBC_BASE_ADDR, 5));
    pst_usbc0_pwr_mng_eve_en_l->bRESUME_INTEN = 1;
    pst_usbc0_pwr_mng_eve_en_l->bRESET_INTEN = 1;
    ms_writeb(eve_en_l, GET_REG16_ADDR(USBC_BASE_ADDR, 5));
...
void msb250x_udc_deinit_utmi(void)
{
    ...
    ms_writew(ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x03)) | BIT0 | BIT1 | BIT8, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x03));
    ms_writew(ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x08)) | BIT12, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x08));
    mdelay(1);
    // clear reset
    ...
    ms_writew(0x7F03, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x0));
    mdelay(5);
...
void msb250x_udc_init_otg(struct msb250x_udc *udc)
{
    ...
    */
    ...
    /* USB Suspend IRQ Enable */
    usbe_intr = ms_readb(MSB250X_OTG0_INTRUSBE_REG);
    pst_intr_usbe->bSuspend = 1;
    ms_writeb(usbe_intr, MSB250X_OTG0_INTRUSBE_REG);
...
void tx_swing_set(u8 value)
{
    u8 reg_value;

    reg_value = ms_readb(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    if (value == 80)
    {
        ms_writeb((reg_value & 0x8F) | 0x40, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 85)
    {
        ms_writeb((reg_value & 0x8F) | 0x50, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 90)
    {
        ms_writeb((reg_value & 0x8F) | 0x60, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 95)
    {
        ms_writeb((reg_value & 0x8F) | 0x70, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 100)
    {
        ms_writeb((reg_value & 0x8F) | 0x00, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 105)
    {
        ms_writeb((reg_value & 0x8F) | 0x10, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 110)
    {
        ms_writeb((reg_value & 0x8F) | 0x20, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 115)
    {
        ms_writeb((reg_value & 0x8F) | 0x30, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else
    {
        printk(KERN_ERR "unsupport tx swing value\n");
    }
}

static ssize_t tx_swing_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
    u8 value;
...
static ssize_t tx_swing_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    u8 value;

    value = (ms_readb(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16)) & 0x70);
    value = value >> 4;
...
static DEVICE_ATTR(tx_swing, 0644, tx_swing_show, tx_swing_store);

void cm_current_set(u8 value)
{
    u8 reg_value;

    reg_value = ms_readb(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17));

    if (value == 105)
    {
        ms_writeb((reg_value & 0xc7) | (0x3 << 3), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17));
    }
    else if (value == 110)
    {
        ms_writeb((reg_value & 0xc7) | (0x2 << 3), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17));
    }
    else if (value == 115)
    {
        ms_writeb((reg_value & 0xc7) | (0x1 << 3), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17));
    }
    else if (value == 120)
    {
        ms_writeb((reg_value & 0xc7) | (0x0 << 3), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17));
    }
    else if (value == 125)
    {
        ms_writeb((reg_value & 0xc7) | (0x5 << 3), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17));
    }
    else if (value == 130)
    {
        ms_writeb((reg_value & 0xc7) | (0x4 << 3), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17));
    }
    else
    {
        printk(KERN_ERR "unsupport cm current value\n");
    }
}


static ssize_t cm_current_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
    u8 value;

    value = simple_strtoul(buf, NULL, 10);
    cm_current_set(value);

    return count;
}

static ssize_t cm_current_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    u8 value;

    value = (ms_readb(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x17)) & 0x38);
    value = value >> 3;

    if (value == 0 || value == 0x6)
    {
        return sprintf(buf, "%s\n", "cm current: 120%");
    }
    else if (value == 0x1 || value == 0x7)
    {
        return sprintf(buf, "%s\n", "cm current: 115%");
    }
    else if (value == 0x2)
...
static DEVICE_ATTR(cm_current, 0644, cm_current_show, cm_current_store);

void de_emphasis_current_set(u8 value)
{
    u16 reg_value;

    reg_value = ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));

    if (value == 100)
    {
        ms_writew((reg_value & 0xfc7f) | (0x0 << 7), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 105)
    {
        ms_writew((reg_value & 0xfc7f) | (0x4 << 7), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 110)
    {
        ms_writew((reg_value & 0xfc7f) | (0x5 << 7), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 115)
    {
        ms_writew((reg_value & 0xfc7f) | (0x6 << 7), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else if (value == 120)
    {
        ms_writew((reg_value & 0xfc7f) | (0x7 << 7), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
    }
    else
    {
        printk(KERN_ERR "unsupport de_emphasis_current value\n");
    }

}

static ssize_t de_emphasis_current_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t count)
{
    u8 value;

    value = simple_strtoul(buf, NULL, 10);
    de_emphasis_current_set(value);

    return count;
}

static ssize_t de_emphasis_current_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    u16 value;

    value = (ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16)) & 0x0380);
    value = value >> 7;

    if (value == 0 || value == 0x1 || value == 0x2 || value == 0x3)
    {
        return sprintf(buf, "%s\n", "de_emphasis_current: 100%");
    }
    else if (value == 0x4)
    {
        return sprintf(buf, "%s\n", "de_emphasis_current: 105%");
    }
    else if (value == 0x5)
    {
        return sprintf(buf, "%s\n", "de_emphasis_current: 110%");
    }
    else if (value == 0x6)
    {
        return sprintf(buf, "%s\n", "de_emphasis_current: 115%");
    }
    else if (value == 0x7)
    {
        return sprintf(buf, "%s\n", "de_emphasis_current: 120%");
    }
    else
    {
        return sprintf(buf, "%s\n", "de_emphasis_current: unknown");
    }
}

static DEVICE_ATTR(de_emphasis_current, 0644, de_emphasis_current_show, de_emphasis_current_store);
...
static int msb250x_udc_probe(struct platform_device *pdev)
{
    ...
    device_create_file(&pdev->dev, &dev_attr_tx_swing);
    device_create_file(&pdev->dev, &dev_attr_cm_current);
    device_create_file(&pdev->dev, &dev_attr_de_emphasis_current);
...
static int msb250x_udc_remove(struct platform_device *pdev)
{
    ...
    device_remove_file(&pdev->dev, &dev_attr_cm_current);
    device_remove_file(&pdev->dev, &dev_attr_de_emphasis_current);
...
/* support wakeup xtal do nothing in system suspend/resume */
#if defined(CONFIG_PM) &&  !defined(USB_SUPPORT_WAKEUP_XTAL)
