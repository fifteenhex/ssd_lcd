...
//#define CONFIG_MSYS_DMEM_SYSFS_ALL
...
//  MSYS_PRINT("\nFREEING DMEM [%s]\n\n",mem_info->name);
    if(mem_info->name[0]!=0)
    {
        list_for_each(ptr, &kept_mem_head)
        {
            int res=0;
            entry = list_entry(ptr, struct DMEM_INFO_LIST, list);
            res=strncmp(entry->dmem_info.name, mem_info->name,strnlen(mem_info->name,31));
...
int msys_read_uuid(unsigned long long* udid)
{
    ...
    *udid = (u64)INREG16(BASE_REG_OTP_PA + REG_ID_54) |
            ((u64)INREG16(BASE_REG_OTP_PA + REG_ID_55) << 16) |
...
extern char __initdata atags_copy[1024];

char* msys_get_iplVersion(void)
{
    /*atag*/
    return atags_copy;
}
...
static ssize_t ms_dump_reboot_type(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    str += scnprintf(str, end - str, "Reboot_Type: %d (", msys_get_rebootType());
    switch(msys_get_rebootType())
    {
        case MSYS_REBOOT_BY_WDT_RST:
            str += scnprintf(str, end - str, "WDT");
            break;
        case MSYS_REBOOT_BY_SW_RST:
            str += scnprintf(str, end - str, "SW");
            break;
        case MSYS_REBOOT_BY_HW_RST:
            str += scnprintf(str, end - str, "HW");
            break;
        default:
            break;
    }
...
static ssize_t ms_dump_ipl_version(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *str = buf;
    char *end = buf + PAGE_SIZE;

    str += scnprintf(str, end - str, "IPL_Version: %s\n", msys_get_iplVersion());

    return (str - buf);
}

DEVICE_ATTR(IPL_VERSION, 0444, ms_dump_ipl_version, NULL);
...
static int __init msys_init(void)
{
    ...
    device_create_file(sys_dev.this_device, &dev_attr_IPL_VERSION);
