...
#define INFINITY6E_CHIP_ID      0xF1
/////////////////////////////////
// to be refined
/////////////////////////////////
#define TXD_NUM         0
...
#if SCATTER_DMA
#define SC_IDX_MAX      512
int SC_DMA_BYPASS_SIZE = 0;
int SC_idx = 0;
#endif
...
#ifdef SCATTER_DMA_INTERRUPT
    MHal_EMAC_SG_IRQ_ENABLE(hemac->hal, 1);
#endif
...
u8 MDev_EMAC_ClkDisable(struct emac_handle *hemac)
{
#ifdef CONFIG_CAM_CLK
    u32 u32clknum = 0;

    for(u32clknum = 0; u32clknum < hemac->EmacParentCnt; u32clknum++)
    {
        if (hemac->pvclk[u32clknum])
        {
            CamClkSetOnOff(hemac->pvclk[u32clknum],0);
        }
    }
#endif
    return 1;
}
u8 MDev_EMAC_ClkEnable(struct emac_handle *hemac)
{
#ifdef CONFIG_CAM_CLK
    u32 u32clknum = 0;
    ...
    for(u32clknum = 0; u32clknum < hemac->EmacParentCnt; u32clknum++)
    {
        if (hemac->pvclk[u32clknum])
        {
            CamClkAttrGet(hemac->pvclk[u32clknum],&stGetCfg);
            CAMCLK_SETPARENT(stSetCfg,stGetCfg.u32Parent[0]);
            CamClkAttrSet(hemac->pvclk[u32clknum],&stSetCfg);
            CamClkSetOnOff(hemac->pvclk[u32clknum],1);
        }
    }
...
u8 MDev_EMAC_ClkRegister(struct emac_handle *hemac, struct device *dev)
{
#ifdef CONFIG_CAM_CLK
    u32 u32clknum;
    u32 EmacClk;
    ...
    if(of_find_property(dev->of_node,"camclk",&hemac->EmacParentCnt))
    {
        hemac->EmacParentCnt /= sizeof(int);
        //printk( "[%s] Number : %d\n", __func__, num_parents);
        if(hemac->EmacParentCnt < 0)
        {
            printk( "[%s] Fail to get parent count! Error Number : %d\n", __func__, hemac->EmacParentCnt);
            return 0;
        }
        hemac->pvclk = kzalloc((sizeof(void *) * hemac->EmacParentCnt), GFP_KERNEL);
        if(!hemac->pvclk){
            return 0;
        }
        for(u32clknum = 0; u32clknum < hemac->EmacParentCnt; u32clknum++)
        {
            EmacClk = 0;
            of_property_read_u32_index(dev->of_node,"camclk", u32clknum,&(EmacClk));
            if (!EmacClk)
            {
            ...
            {
                CamOsSnprintf(str, 8, "emac_%d ",u32clknum);
                CamClkRegister(str,EmacClk,&(hemac->pvclk[u32clknum]));
            }
        }
    }
    else
...
u8 MDev_EMAC_ClkUnregister(struct emac_handle *hemac)
{
#ifdef CONFIG_CAM_CLK
    u32 u32clknum;
    ...
    for(u32clknum=0;u32clknum<hemac->EmacParentCnt;u32clknum++)
    {
        if(hemac->pvclk[u32clknum])
        {
            printk(KERN_DEBUG "[%s] %p\n", __func__,hemac->pvclk[u32clknum]);
            CamClkUnregister(hemac->pvclk[u32clknum]);
            hemac->pvclk[u32clknum] = NULL;
        }
    }
    kfree(hemac->pvclk);
#endif
...
static int _MDev_EMAC_tx_pump(struct emac_handle *hemac, int bFree, int bPump)
{
    ...
    if (bPump)
    {
        ...
        for (i = 0; i < nPkt; i++)
        {
            ...
            #if (!SCATTER_DMA) || (SCATTER_DMA && SCATTER_DMA_BYPASS)
            if (skb_addr)
            #endif
            {
...
#if DYNAMIC_INT_TX
    #if TX_DELAY_INT
        MHal_EMAC_IntEnable(hemac->hal, EMAC_INT_TX_DELAY, 1);
        MHal_EMAC_IntEnable(hemac->hal, EMAC_INT_TX_DELAY_TIMEOUT, 1024);
        MHal_EMAC_WritReg32(hemac->hal, REG_TXDELAY_MAX_CNT, 16);
    #else
...
#if (!SCATTER_DMA) || (SCATTER_DMA && SCATTER_DMA_BYPASS) //ORIGINAL OR SCATTER DMA BYPASS
#if EMAC_SG
    {
        int i;
...
#if SCATTER_DMA && !SCATTER_DMA_BYPASS
    {
        int i;
        int nr_frags = skb_shinfo(skb)->nr_frags;
        struct sk_buff* skb_tmp = skb_clone(skb, GFP_ATOMIC);
        ...
        if (nr_frags)
        {
            ...
            Chip_Flush_Cache_Range((size_t)start, skb_headlen(skb));
            skb_addr = VIRT2BUS(start);
            MHal_EMAC_SG_WRITE(hemac->hal, skb_addr, skb_headlen(skb), nr_frags, 0xFF);
#if SCATTER_DMA_V3
            MHal_EMAC_SG_V3_FRAG_FILL(skb_addr, skb_headlen(skb), SC_idx, 0);
#endif
            ...
            for (i = 0; i < nr_frags; i++)
            {
                ...
                Chip_Flush_Cache_Range((size_t)frag_addr, skb_frag_size(frag));
                skb_addr = VIRT2BUS(frag_addr);
                MHal_EMAC_SG_WRITE(hemac->hal, skb_addr, skb_frag_size(frag), nr_frags, i+1);
#if SCATTER_DMA_V3
                MHal_EMAC_SG_V3_FRAG_FILL(skb_addr, skb_frag_size(frag), (SC_idx+1+i)%SC_IDX_MAX, ((i+1) == nr_frags)? 1:0);
#endif
            }

            skb_addr = (SC_idx<<20)|((nr_frags)<<16);

            spin_lock_irqsave(&hemac->mutexTXQ, flags);
            skb_queue_insert(&(hemac->skb_queue_tx), skb, (dma_addr_t)skb_addr, skb->len, 1);
            spin_unlock_irqrestore(&hemac->mutexTXQ, flags);

            SC_idx += (nr_frags+1);
            if(SC_idx >= SC_IDX_MAX)
            {
                SC_idx = (SC_idx%SC_IDX_MAX);
            }
        }
        else
        {
            Chip_Flush_Cache_Range((size_t)skb->data, skb->len);
            skb_addr = VIRT2BUS(skb->data);
            MHal_EMAC_SG_WRITE(hemac->hal, skb_addr, skb->len, 0, 0);
#if SCATTER_DMA_V3
            MHal_EMAC_SG_V3_FRAG_FILL(skb_addr, skb->len, SC_idx, 1);
#endif
            skb_addr = (SC_idx<<20)|((nr_frags)<<16);
...
static u32 _MDrv_EMAC_ISR_TDLY(struct net_device *dev, u32 intstatus)
{
    ...
    MHal_EMAC_IntEnable(hemac->hal, EMAC_INT_TX_DELAY, 0);
    MHal_EMAC_IntEnable(hemac->hal, EMAC_INT_TX_DELAY_TIMEOUT, 0);
...
static u32 _MDrv_EMAC_ISR_TDTO(struct net_device *dev, u32 intstatus)
{
    ...
    tx_delay_pack_cnt = MHal_EMAC_PACKET_CNT(hemac->hal);
...
#ifdef SCATTER_DMA_INTERRUPT
irqreturn_t MDev_EMAC_interrupt_ScatterDMA(int irq,void *dev_id)
{
    struct net_device *dev = (struct net_device *) dev_id;
    struct emac_handle *hemac = (struct emac_handle*) netdev_priv(dev);

    //MHal_EMAC_SG_IRQ_ENABLE(hemac->hal, 0);
    MHal_EMAC_SG_IRQ_CLEAR(hemac->hal);
    MHal_EMAC_SG_RESET(hemac->hal);
...
#if SCATTER_DMA
    MHal_EMAC_RX_MIU_ADDR(hemac->hal, VIRT2BUS(rxinfo->desc), SC_DMA_BYPASS_SIZE);
#if SCATTER_DMA_BYPASS
    MHal_EMAC_SG_BYPASS(hemac->hal, TRUE);
#else
    MHal_EMAC_SG_BYPASS(hemac->hal, FALSE);
#if SCATTER_DMA_V3
    MHal_EMAC_SG_V3(hemac->hal, TRUE);
    MHal_EMAC_SG_V3_TABLE_ADDR(hemac->hal, 0xA0004000);
    MHal_EMAC_SG_V3_PACKET_ADDR(hemac->hal, 0xA0005000);
#else
    MHal_EMAC_SG_V3(hemac->hal, FALSE);
#endif
#endif
#endif
...
#if SCATTER_DMA
    SC_DMA_BYPASS_SIZE = hemac->mem_info.length;
#endif
...
static ssize_t info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    clk = MHal_EMAC_ReadReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN0, 0x84);
    clk = clk & 0x0F;
    if(clk == 0)
...
int _StrSplit(char **arr, char *str,  char* del)
{
    char *cur = str;
    char *token = NULL;
    int cnt = 0;
...
static int MDev_EMAC_setup (struct net_device *dev)
{
...
#ifdef SCATTER_DMA_INTERRUPT
    if (!(hemac->irq_scatterDMA))
    {
        EMAC_ERR("Get emac scatterDMA irq number error from DTS\n");
        return -EPROBE_DEFER;
    }

    if (request_irq(hemac->irq_scatterDMA, MDev_EMAC_interrupt_ScatterDMA, 0, dev->name, dev))
    {
        EMAC_ERR("Request emac scatterDMA irq fail\n");
        return -EBUSY;
    }
#endif
    ...
    device_create_file(hemac->mstar_class_emac_device, &dev_attr_phyStatusWR);
#if 0//ajtest
...
////////////////////////////////////////////////////////////////////////////
    ...
    add_timer(&RX_timer);
#endif
    MDev_EMAC_ClkRegister(hemac,&pdev->dev);
    MDev_EMAC_ClkEnable(hemac);
    MHal_EMAC_Power_On_Clk(hemac->hal, &pdev->dev);

    if (hemac->led_invet && hemac->phyRIU)
    {
        MHal_EMAC_Set_Invert_LED(hemac->hal);
    }

    if (hemac->led_reverse && hemac->phyRIU)
    {
        MHal_EMAC_Set_Reverse_LED(hemac->hal, hemac->led_reverse);
    }
...
static void MDev_EMAC_dts(struct net_device* netdev)
{
...
#ifdef SCATTER_DMA_INTERRUPT
    hemac->irq_scatterDMA = irq_of_parse_and_map(netdev->dev.of_node, 2);
#endif
    // printk("[%s][%d] (irq_emac, irq_lan) = (%d, %d)\n", __FUNCTION__, __LINE__, hemac->irq_emac, hemac->irq_lan);
    ...
    {
       hemac->led_invet = (unsigned char)led_data;
        // printk(KERN_ERR "[EMAC]Set emac led-invert=%d\n",led_data);
    }

    if(!of_property_read_u32(netdev->dev.of_node, "led-reverse", &led_data))
    {
       hemac->led_reverse = (unsigned char)led_data;
        // printk(KERN_ERR "[EMAC]Set emac led-invert=%d\n",led_data);
    }
    ...
    {
        if(CamOsChipId() == INFINITY6E_CHIP_ID) //I6e
        {
            hemac->phyRIU = 0x1F2A2800;
        }
        else
        {
