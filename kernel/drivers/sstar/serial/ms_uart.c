...
static s32 _ms_uart_console_prepare(int idx)
{
    ...
    lpae_base = (unsigned int)res.start;
    console_port.port.membase = (void *)lpae_base;
...
static unsigned short pad_mod_saved = 0;
static ssize_t console_uart_pad_disable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *str = buf;
    char *end = buf + PAGE_SIZE;

    str += scnprintf(str, end - str, "UART PAD %s (mode saved: %d)\n", (INREGMSK16(0x1F203D4C, 0x000F) == 0) ? "Disabled": "Enabled", pad_mod_saved);
    return (str - buf);
}

static ssize_t console_uart_pad_disable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t n)
{
    if(NULL!=buf)
    {
        size_t len;
        ...
        if(1==len)
        {
            if('0'==buf[0]) // enable
            {
                OUTREGMSK16(0x1F203D4C, pad_mod_saved, 0x000F);
            }
            else if('1'==buf[0]) // disable
            {
                pad_mod_saved = INREGMSK16(0x1F203D4C, 0x000F);
                OUTREGMSK16(0x1F203D4C, 0x0, 0x000F);
            }
            return n;
        }
        return -EINVAL;
...
DEVICE_ATTR(console_disable, 0644, console_uart_pad_disable_show, console_uart_pad_disable_store);
...
//        printk(KERN_ERR "DMA_TX_STOP\n");
    ...
    {
        while( !( INREG8(REG_LSR(p)) & UART_LSR_TEMT ) );
...
#define __UART_RAW_PUT_STR(x) \
    { \
        ...
        { \
            while(!(INREG8(REG_LSR(p)) & UART_LSR_THRE)); \
            OUTREG8(REG_DLL_THR_RBR(p),str[i]); \
            i++; \
        } \
    }
...
static void ms_getchar(struct uart_port *p)
{
    ...
    unsigned long flags;
    bool bConsole = (mp->port.membase == console_port.port.membase) ? true : false;
    ...
    do{
        ...
        if (bConsole)
        {
            ...
            if(rx_disable == 5)
            {
                ...
                if (silent_state)
                {
                    __UART_RAW_PUT_STR("disable uart\n");
                    ch = 0;
                }
                else
                {
                    __UART_RAW_PUT_STR("enable uart\n");
                }
...
static s32 ms_uart_suspend(struct platform_device *pdev, pm_message_t state)
{
    ...
    if ((console_suspend_enabled) || (p->line != 0))
    {
        if(mp->use_dma)
        {
            URDMA_RxEnable(p,FALSE);
...
static s32 ms_uart_probe(struct platform_device *pdev)
{
   ...
   /*if (mp->port.line < 0) {
    ...
    lpae_base = (unsigned int)res->start;
    mp->port.membase = (void *)lpae_base;
...
dma_err:
    ...
    if(mp->port.line==0)
    {
        ...
        device_create_file(&pdev->dev, &dev_attr_console_disable);
    }
