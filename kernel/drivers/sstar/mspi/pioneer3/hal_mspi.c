...
#define MSPI_BDMA_SEL                      0xFD200E50
#define BDMA_SEL                           0x11
...
{
    WRITE_WORD(MSPI_BDMA_SEL, BDMA_SEL);
    HAL_MSPI_Lock();
...
u8 HAL_MSPI_Set3WireMode(struct mspi_hal *mspi, u8 enable)
{
    ...
    MSPI_WRITE_MASK(MSPI_CTRL_OFFSET, enable, MSPI_CLK_3WIREMODE_MASK);
    ...
    for(u8Index = 0; u8Index < u16Size; u8Index++)
    {
        if(u8Index & 1)
        {
            ...
            else
            {
                ...
                if(isMsbBitMode)
                {
                    u16TempBuf = ((tx_buff[u8Index] & (0xFF >> shift))<< shift) | (tx_buff[u8Index-1] << 8);
                }
                else
                {
                    //NO LSB
...
{
    u32 u32Index = 0;
    u32 u32TempFrameCnt=0;
    u32 u32TempLastFrameCnt=0;
    u8  ret = E_MSPI_OK;
    ...
    {
        ret = HAL_MSPI_WriteBuf(mspi,pData+u32Index*MAX_WRITE_BUF_SIZE,MAX_WRITE_BUF_SIZE);
        if (ret)
        {
            return E_MSPI_OPERATION_ERROR;
        }
    ...
    {
        ret = HAL_MSPI_WriteBuf(mspi,pData+u32TempFrameCnt*MAX_WRITE_BUF_SIZE,u32TempLastFrameCnt);
    }
...
u8 HAL_MSPI_Read(u8 u8Channel, struct mspi_hal *mspi, u8 *pData, u32 u32Size)
{
    ...
    for (u32Index = 0; u32Index < u32TempFrameCnt; u32Index++)
    {
        ret = HAL_MSPI_ReadBuf(mspi,pData+u32Index*MAX_WRITE_BUF_SIZE,MAX_WRITE_BUF_SIZE);
        if (ret)
        {
            return E_MSPI_OPERATION_ERROR;
        }
    ...
    if(u32TempLastFrameCnt)
    {
        ret = HAL_MSPI_ReadBuf(mspi,pData+u32TempFrameCnt*MAX_WRITE_BUF_SIZE,u32TempLastFrameCnt);
    }
    if (ret)
    {
        return E_MSPI_OPERATION_ERROR;
...
u8 HAL_MSPI_DMA_Write(u8       u8Channel, struct mspi_hal *mspi, u8 *pData, u32 u32Size)
{
    ...
    tBdmaParam.ePathSel          = (u8Channel?HAL_BDMA_MIU0_TO_MSPI1:HAL_BDMA_MIU0_TO_MSPI0); //need check
    tBdmaParam.eDstAddrMode      = HAL_BDMA_ADDR_INC; //address increase
    tBdmaParam.u32TxCount        = (u32)u32Size;
    tBdmaParam.u32Pattern        = 0;
    tBdmaParam.pSrcAddr          = (void*)data_addr;
    tBdmaParam.pfTxCbFunc        = NULL;
    Chip_Flush_Cache_Range((u32)pData, u32Size);
...
u8 HAL_MSPI_DMA_Read(u8 u8Channel, struct mspi_hal *mspi, u8 *pData, u32 u32Size)
{
    ...
    memset(&tBdmaParam, 0, sizeof(HalBdmaParam_t));
    tBdmaParam.bIntMode          = 1; //0:use polling mode
    tBdmaParam.ePathSel          = (u8Channel?HAL_BDMA_MSPI1_TO_MIU0:HAL_BDMA_MSPI0_TO_MIU0); //need check
    tBdmaParam.eDstAddrMode      = HAL_BDMA_ADDR_INC; //address increase
    tBdmaParam.u32TxCount        = (u32)u32Size;
    tBdmaParam.u32Pattern        = 0;
    tBdmaParam.pDstAddr          = (void*)data_addr;
    tBdmaParam.pfTxCbFunc        = NULL;
    ...
    Chip_Inv_Cache_Range((u32)pData, u32Size);
    errnum = HAL_MSPI_Trigger(mspi);
    if(errnum)
    {
        goto err_out;
