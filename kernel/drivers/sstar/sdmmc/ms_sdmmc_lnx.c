...
static U32_T    gu32_SdioRstDelay[3]        = {0, 0, 0};
...
BOOL_T gb_Sdio_dis_Intr[3]                  = {FALSE, FALSE, FALSE};
...
static void _SwitchPAD(SlotEmType eSlot)
{
    ...
    Hal_Timer_mSleep(gu32_SdioRstDelay[eSlot]);
    if (!bIsFakeCdz)
    {
...
{
    return (gb_SdioUseSlot[eSlot])? FALSE: TRUE;
}
//------------------------------------------------------------------------------------------------
...
static int ms_sdmmc_dts_init(struct platform_device *p_dev)
{
    ...
    if(!u32_SlotNums)
    {
        pr_err(">> [sdmmc] Err: Could not get SD pad group from Padmux dts!\n");
        return 1;
    }
#else
    ...
    if (of_property_read_u32_array(p_dev->dev.of_node, "slot-sdio-rst-delay", (U32_T *)u32_SdioRstDelay, u32_SlotNums))
    {
        pr_err(">> [sdmmc] Err: Could not get dts [slot-sdio-use] option!\n");
        return 1;
    }
    ...
    for (slotNo =0; slotNo<gu8_SlotNums; slotNo++)
    {
        ...
        if (mdrv_padmux_active() != 0)
        {
            ...
            {
                if(!u32_IntCDZSlot[eSlot])
                {
                    pr_sd_dbg(">> [sdmmc] Dbg: slotNo = %u, the slot'cdz is use poll mode, and no need cdz pad pin!\n", slotNo);
                }
                else
                {
            ...
            {
                if(u32_SdioUseSlot[eSlot])
                {
                    pr_sd_dbg(">> [sdmmc] Dbg: slotNo = %u, the slot is used for sdio, and no need power pad pin!\n", slotNo);
                }
                else
                {
        ...
        gu32_SdioRstDelay[eSlot] = (BOOL_T)u32_SdioRstDelay[eSlot];
        gb_RemovableSlot[eSlot] = (BOOL_T)u32_RemovableSlot[eSlot];
        gb_Sdio_Use_1bit[eSlot] = (BOOL_T)u32_Sdio_Use_1bit[eSlot];
        gb_Sdio_dis_Intr[eSlot] = (BOOL_T)u32_Sdio_dis_intr[eSlot];
    ...
    pr_sd_dbg(">> [sdmmc] gu32_SdioRstDelay[0-2]= %u, %u, %u \n", gu32_SdioRstDelay[0], gu32_SdioRstDelay[1], gu32_SdioRstDelay[2]);
    pr_sd_dbg(">> [sdmmc] gb_RemovableSlot[0-2]= %u, %u, %u \n", gb_RemovableSlot[0], gb_RemovableSlot[1], gb_RemovableSlot[2]);
    pr_sd_dbg(">> [sdmmc] gb_Sdio_Use_1bit[0-2]= %u, %u, %u \n", gb_Sdio_Use_1bit[0], gb_Sdio_Use_1bit[1], gb_Sdio_Use_1bit[2]);
    pr_sd_dbg(">> [sdmmc] gb_Sdio_dis_Intr[0-2]= %u, %u, %u \n", gb_Sdio_dis_Intr[0], gb_Sdio_dis_Intr[1], gb_Sdio_dis_Intr[2]);
...
static ssize_t sdmmc0_get_clk_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%dKHz\n", gu32_SdmmcClk[0]/1000);
}

DEVICE_ATTR(get_sdmmc0_clock, S_IRUSR, sdmmc0_get_clk_show, NULL);

static ssize_t sdmmc1_get_clk_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%dKHz\n", gu32_SdmmcClk[1]/1000);
}

DEVICE_ATTR(get_sdmmc1_clock, S_IRUSR, sdmmc1_get_clk_show, NULL);

static ssize_t sdmmc2_get_clk_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    return sprintf(buf, "%dKHz\n", gu32_SdmmcClk[2]/1000);
}

DEVICE_ATTR(get_sdmmc2_clock, S_IRUSR, sdmmc2_get_clk_show, NULL);

static struct attribute *mstar_sdmmc0_attr[] =
{
    &dev_attr_get_sdmmc0_clock.attr,
    NULL,
};

static struct attribute *mstar_sdmmc1_attr[] =
{
    &dev_attr_get_sdmmc1_clock.attr,
    NULL,
};

static struct attribute *mstar_sdmmc2_attr[] =
{
    &dev_attr_get_sdmmc2_clock.attr,
    NULL,
};

static struct attribute_group mstar_sdmmc_attr_grp[3] =
{
    {.attrs = mstar_sdmmc0_attr,},   \
    {.attrs = mstar_sdmmc1_attr,},   \
    {.attrs = mstar_sdmmc2_attr,},
};
...
//###########################################################################################################
    ...
    nRet = sysfs_create_group(&p_sdmmc_host->pdev->dev.kobj, &mstar_sdmmc_attr_grp[eSlot]);
...
//###########################################################################################################
    ...
    //
    sysfs_remove_group(&p_sdmmc_host->pdev->dev.kobj, &mstar_sdmmc_attr_grp[eSlot]);
