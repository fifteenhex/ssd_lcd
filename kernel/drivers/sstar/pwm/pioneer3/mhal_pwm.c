...
//    }
    ...
    DrvPWMDutyQE0(ms_chip, 0); //to avoid puse in booting sync 4.19@e9a47bcece1a6
#else
...
//    pwmclk = (U32)(clk_get_rate(ms_chip->clk));
    ...
    u32Period--;
    u32Duty--;
    pr_err("source clk= %d, u16Div=%d u32Duty=0x%x u32Period=0x%x\n", ms_chip->clk_frequence, u16Div, u32Duty, u32Period);
...
void DrvPWMSuspend(struct mstar_pwm_chip *ms_chip,int u8Id)
{
    if(ms_chip->base)
        SETREG16(ms_chip->base + u16REG_SW_RESET, 1<<u8Id);
}

void DrvPWMResume(struct mstar_pwm_chip *ms_chip,int u8Id)
{
    #ifdef CONFIG_PWM_NEW
    if(ms_chip->base){
        DrvPWMSetConfig(ms_chip, u8Id, _pwmDuty_ns[u8Id], _pwmPeriod_ns[u8Id]);
    }
    #else
    DrvPWMSetDuty(ms_chip, u8Id, _pwmDuty[u8Id]);
    DrvPWMSetPeriod(ms_chip, u8Id, _pwmPeriod[u8Id]);
    #endif
}
#endif
...
int DrvPWMDutyQE0(struct mstar_pwm_chip *ms_chip, U8 u8Val)
{
#if 1
    printk("[%s] grp:%d\n", __FUNCTION__, u8Val);
    if (u8Val)
        SETREG16(ms_chip->base + REG_PWM_DUTY_QE0, (1 << (REG_PWM_DUTY_QE0_SHFT)));
    else
        CLRREG16(ms_chip->base + REG_PWM_DUTY_QE0, (1 << (REG_PWM_DUTY_QE0_SHFT)));
#else
