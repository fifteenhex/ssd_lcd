...
#if CLK_DEBUG
#define CLK_DBG(fmt, arg...) printk(KERN_ERR fmt, ##arg)
#else
...
static unsigned long  ss_spipll_recalc_rate(struct clk_hw *clk_hw, unsigned long parent_rate)
{

    if(INREG16(BASE_REG_RIU_PA + (0x103900 << 1))==0x7627){
        return 104000000;
    }
    else if (INREG16(BASE_REG_RIU_PA + (0x103900 << 1))==0x2FA0)
        return 86000000;
    else
        return 0;
}

static long ss_spipll_round_rate(struct clk_hw *clk_hw, unsigned long rate, unsigned long *parent_rate)
{
    if(rate <= 86000000)
    {
        return 86000000;
...
void ss_pll_init(struct clk_hw *clk_hw)
{
    return;
}
static int ss_spipll_set_rate(struct clk_hw *clk_hw, unsigned long rate, unsigned long parent_rate)
{
    int ret = 0;

    if(rate==86000000)
    {
        OUTREG16(BASE_REG_RIU_PA + (0x103900 << 1), 0x2FA0);
        OUTREG16(BASE_REG_RIU_PA + (0x103902 << 1), 0x0028);
        OUTREG16(BASE_REG_RIU_PA + (0x103966 << 1), 0x0000);
        OUTREG16(BASE_REG_RIU_PA + (0x103968 << 1), 0x0002);
        OUTREG16(BASE_REG_RIU_PA + (0x10396E << 1), 0x0002);
        OUTREG16(BASE_REG_RIU_PA + (0x103970 << 1), 0x0000);
        CLK_ERR("[%s] Set spipll synth rate to %lu !!!\r\n", __FUNCTION__, rate);
    }
    else if(rate==104000000)
    {   //from IPL PalSpiflash_SetClk
        OUTREG16(BASE_REG_RIU_PA + (0x103900 << 1), 0x7627);//DFFSET
        OUTREG16(BASE_REG_RIU_PA + (0x103902 << 1), 0x0042);//DFFSET
        OUTREG16(BASE_REG_RIU_PA + (0x10394E << 1), 0x0001);//synthpll pdiv
        OUTREG16(BASE_REG_RIU_PA + (0x10396C << 1), 0x0000);//SYNTHPLL_PD
        CLK_ERR("[%s] Set spipll synth rate to %lu !!!\r\n", __FUNCTION__, rate);
    }
    else
    {
        CLK_ERR("[%s] Not support rate %lu \r\n", __FUNCTION__, rate);
    }
    return ret;
}
static int ss_spipll_enable(struct clk_hw *hw)
{
    CLK_DBG("\nss_spipll_enable\n\n");
    return 0;
}

static void ss_spipll_disable(struct clk_hw *hw)
{
    CLK_DBG("\nss_spipll_disable\n\n");
}


struct clk_ops ss_pllclk_ops = {
    .enable=ss_spipll_enable,
    .disable=ss_spipll_disable,
    .round_rate = ss_spipll_round_rate,
    .recalc_rate = ss_spipll_recalc_rate,
    .set_rate = ss_spipll_set_rate,
    .init = ss_pll_init,
};
...
static void __init ms_clk_complex_init(struct device_node *node)
{
    ...
    else if(!strcmp(node->name, "CLK_spi_synth_pll"))
    {
        CLK_ERR("Find %s, hook ss_pllclk_ops\n", node->name);
        init->ops = &ss_pllclk_ops;
    }
    else
    {
