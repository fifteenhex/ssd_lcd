...
#define REG_ADDR_UBOOT_FLAG       (REG_ADDR_BASE_MAILBOX + (0x01 << 2))
#define UBOOT_FLAG_MAGIC_MASK     0xFF00
#define UBOOT_FLAG_MAGIC          0xDD00
#define UBOOT_FLAG_GET_MAGIC(x)   (*(volatile unsigned short *)(x) & UBOOT_FLAG_MAGIC)
#define UBOOT_FLAG_SILENT_CONSOLE 0x01

bool console_get_silent_env_shadow(void)
{
    if( *(volatile unsigned short *)(REG_ADDR_UBOOT_FLAG) & UBOOT_FLAG_SILENT_CONSOLE &&
      UBOOT_FLAG_GET_MAGIC(REG_ADDR_UBOOT_FLAG) == UBOOT_FLAG_MAGIC )
    {
        debug("%s: silent shadow -> 1\n", __func__);
        return true;
    }
    else
    {
        debug("%s: silent shadow -> 0\n", __func__);
        return false;
    }
}
...
extern unsigned long long getUUID(void);
unsigned long long getUUID(void)
{
    unsigned long long uuid=0;
    CLRREG16(0x1f00400c, BIT8);
    uuid = (u64)INREG16(0x1F004058) | ((u64)(INREG16(0x1F00405c))<<16) | ((u64)(INREG16(0x1F004060)) << 32);
