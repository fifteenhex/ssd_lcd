...
#define HAL_GOP_PIX2BYTE(x, fmt)     (fmt)==E_HAL_GOP_GWIN_SRC_ARGB8888     || \
                                     ...
                                     (fmt)==E_HAL_GOP_GWIN_SRC_I2_PALETTE   ? (x) / 4 : \
                                                                             (x)
#define PARSING_CSC_TYPE(x) (\
                             x == E_HAL_GOP_CSC_OFF ? "OFF" : \
                             x == E_HAL_GOP_CSC_R2Y ? "R2Y" : \
                             x == E_HAL_GOP_CSC_Y2R ? "Y2R" : \
                                                      "UNKNOWN")
...
HalGopCscType_e geCscType[HAL_RGN_GOP_NUM] =
{
    E_HAL_GOP_CSC_OFF,
    E_HAL_GOP_CSC_OFF,
    E_HAL_GOP_CSC_OFF,
...
HalGopOutFormatType_e geOutFormat[HAL_RGN_GOP_NUM] =
{
...
static MS_U32 _HalGopGetGwinBaseAddr(HalGopIdType_e eGopId, HalGopGwinIdType_e eGwinId)
{
    ...
    {
        HALRGNERR("[%s]Wrong eGwinId=%d\n",__FUNCTION__,eGwinId);
        CamOsPanic("");
    }
...
static HalGopGwinSrcFormatRealVal_e _HalGopSrcFmtTransRealVal(HalGopIdType_e eGopId, HalGopGwinSrcFormat_e eSrcFmt)
{
    ...
    if(HAL_GOP_IS_DISP_UI(eGopId))
    {
        switch(eSrcFmt)
        {
            ...
            default:
                ...
                HALRGNERR("[GOP]%s %d: GOPId=0x%x %s(%x) not support\n", __FUNCTION__, __LINE__, eGopId, PARSING_SRC_FMT(eSrcFmt), eSrcFmt);
                break;
        }
    }
    else
    {
        switch(eSrcFmt)
        {
            ...
            default:
                eVal = E_HAL_GOP_GWIN_SRC_VAL_NOTSUPPORT;
                HALRGNERR("[GOP]%s %d: GOPId=0x%x %s(%d) not support\n", __FUNCTION__, __LINE__, eGopId, PARSING_SRC_FMT(eSrcFmt), eSrcFmt);
                break;
        }
    }
    return eVal;
...
static MS_U16 _HalGopGetBindMode(HalGopIdType_e eGopId, HalGopOutFormatType_e *outFmt)
{
    ...
    {
        (*outFmt) = (geCscType[eGopId] == E_HAL_GOP_CSC_R2Y) ? E_HAL_GOP_OUT_FMT_RGB :
                    (geCscType[eGopId] == E_HAL_GOP_CSC_Y2R) ? E_HAL_GOP_OUT_FMT_YUV :
                                                               geOutFormat[eGopId];
...
{
    bool bRet = 1;
    if((eGopId != E_HAL_GOP_ID_0) &&
       (eGopId != E_HAL_GOP_ID_1) &&
       (eGopId != E_HAL_GOP_ID_7))
    {
        bRet = 0;
    }
...
void HalGopSetGwinSrcFmt(HalGopIdType_e eGopId, HalGopGwinIdType_e eGwinId, HalGopGwinSrcFormat_e eSrcFmt)
{
    ...
    HalGopGwinIdType_e eGwinMax = E_HAL_GOP_GWIN_ID_0; //(HAL_GOP_IS_DISP(eGopId))?E_HAL_GOP_GWIN_ID_0:E_HAL_GOP_GWIN_ID_7;
    ...
    DRVRGNERR("%s %d, GopID:%d, Fmt:%d\n", __FUNCTION__, __LINE__, eGopId, eValSrcFmt);
    for(eGwinIdTmp=E_HAL_GOP_GWIN_ID_0;eGwinIdTmp<=eGwinMax;eGwinIdTmp++)
    {
        ...
        if(HAL_GOP_IS_DISP_UI(eGopId))
        {
            _HalGopGetBindMode(eGopId, &eOutFormat);

            GOP_Reg_Base = _HalGopGetBaseAddr(eGopId);
            HalRgnWrite2ByteMsk(GOP_Reg_Base + REG_GOP_00, eOutFormat == E_HAL_GOP_OUT_FMT_YUV ? 0x0400 : 0x0000, 0x0400, eCmdqId);
        }
    }
    // Double Buffer Write
    _HalGopWriteDoubleBuffer(GOP_Reg_Base,eGopId, eCmdqId);
...
void HalGopSetColorKey(HalGopIdType_e eGopId, HalGopColorKeyConfig_t *ptCfg, bool bVYU)
{
    ...
    if(ptCfg->bEn)
    {
        ...
        HalRgnWrite2ByteMsk(GOP_Reg_Base + REG_GOP_00,
                            bVYU ? GOP_VYU_TRANSPARENT_COLOR_ENABLE : GOP_RGB_TRANSPARENT_COLOR_ENABLE,
    ...
    {
        HalRgnWrite2ByteMsk(GOP_Reg_Base + REG_GOP_00,
                            bVYU ? GOP_VYU_TRANSPARENT_COLOR_DISABLE : GOP_RGB_TRANSPARENT_COLOR_DISABLE,
                            bVYU ? GOP_VYU_TRANSPARENT_COLOR_ENABLE : GOP_RGB_TRANSPARENT_COLOR_ENABLE, eCmdqId);
    }
    // Double Buffer Write
    _HalGopWriteDoubleBuffer(GOP_Reg_Base, eGopId, eCmdqId);
...
void HalGopSetPaletteRiu(HalGopIdType_e eGopId, HalGopPaletteConfig_t *ptCfg)
{
    ...
    if(eGopId == E_HAL_GOP_ID_7)
    {
        HalRgnWrite2ByteMsk(0x113342, 0x0000, 0x0010, eCmdqId); // reg_ckg_gop1_psram
    }
    else
    {
        HalRgnWrite2ByteMsk(0x113942, 0x0000, 0x0001, eCmdqId); // reg_ckg_gop0_psram
    }
...
void HalGopSetCscConfig(HalGopIdType_e eGopId, void *pCfg)
{
    u32 GOP_Reg_Base = 0;
    HalRgnCmdqIdType_e eCmdqId;
    HalGopCscConfig_t *pCscCfg = (HalGopCscConfig_t *)pCfg;
    u16 u16Val;
    HalGopOutFormatType_e eFmt;
    u8 i;

    if(HAL_GOP_IS_DISP_UI(eGopId))
    {
        GOP_Reg_Base = REG_GOPST_BASE;
        _HalGopIdTransCmdqID(eGopId,&eCmdqId);

        if(pCscCfg->u8CoefNum == HAL_RGN_GOP_CSC_COEF_NUM)
        {
            geCscType[eGopId] = pCscCfg->eType;

            u16Val = pCscCfg->eType == E_HAL_GOP_CSC_OFF ? 0 : pCscCfg->pu16Coef[0] | 0x0010;
            HalRgnWrite2ByteMsk(GOP_Reg_Base+REG_GOP_20, u16Val, 0xFFFF, eCmdqId);
            for(i=0; i<9; i++)
            {
                HalRgnWrite2ByteMsk(GOP_Reg_Base+REG_GOP_21+i*2, pCscCfg->pu16Coef[i+1], 0xFFFF, eCmdqId);
            }

            _HalGopGetBindMode(eGopId, &eFmt);
            u16Val = eFmt == E_HAL_GOP_OUT_FMT_RGB ? 0x0000 : 0x0400;
            GOP_Reg_Base = _HalGopGetBaseAddr(eGopId);
            HalRgnWrite2ByteMsk(GOP_Reg_Base + REG_GOP_00, u16Val, 0x0400, eCmdqId);

            HALRGNERR("%s %d, Csc:%s, Out:%s, Param(%04x %04x %x04 %04x %04x %04x %04x %04x %04x %04x)\n",
                __FUNCTION__, __LINE__,
                PARSING_CSC_TYPE(pCscCfg->eType),
                PARSING_OUT_MODE(eFmt),
                pCscCfg->pu16Coef[0], pCscCfg->pu16Coef[1], pCscCfg->pu16Coef[2],
                pCscCfg->pu16Coef[3], pCscCfg->pu16Coef[4], pCscCfg->pu16Coef[5],
                pCscCfg->pu16Coef[6], pCscCfg->pu16Coef[7], pCscCfg->pu16Coef[8],
                pCscCfg->pu16Coef[9]);
        }
        // Double Buffer Write
        _HalGopWriteDoubleBuffer(GOP_Reg_Base,eGopId, eCmdqId);
...
void HalGopSetOutputFormat(HalGopIdType_e eGopId, void *pCfg)
{
    u32 GOP_Reg_Base = 0;
    HalRgnCmdqIdType_e eCmdqId;
    HalGopOutFormatType_e *peOutputFmt = (HalGopOutFormatType_e *)pCfg;
    HalGopOutFormatType_e eFmt;
    u16 u16Val;
    ...
    geOutFormat[eGopId]= *peOutputFmt;
    _HalGopGetBindMode(eGopId, &eFmt);
...
void HalGopGetForceUpdate(HalGopIdType_e eGopId, void *pCfg)
{
    HalGopForceUpdateConfig_t *pstForceCfg = (HalGopForceUpdateConfig_t *)pCfg;
    pstForceCfg->bEn = HAL_GOP_IS_DISP_UI(eGopId) ? 0 : 1;
}

void HalGopTransGopIdToType(HalGopIdType_e eGopId, void *pCfg)
{
    HalGopSrcType_e *eType = (HalGopSrcType_e *)pCfg;
    if(eGopId==E_HAL_GOP_ID_0 || eGopId==E_HAL_GOP_ID_1)
    {
        *eType = E_HAL_GOP_TYPE_SCL;
    }
    else if(eGopId==E_HAL_GOP_ID_7)
    {
        *eType = E_HAL_GOP_TYPE_DISP;
    }
    else
    {
        *eType = E_HAL_GOP_TYPE_MAX;
    }
}
...
void HalGopInit(void)
{
    ...
    gGoppfForSet[E_HAL_QUERY_SETVIDEOTIMINGINFO] = NULL;
    gGoppfForSet[E_HAL_QUERY_SETCSCINFO]         = HalGopSetCscConfig;
    gGoppfForSet[E_HAL_QUERY_SETOUTPUTFORMAT]    = HalGopSetOutputFormat;
    gGoppfForSet[E_HAL_QUERY_GETFORCEUPDATE]     = HalGopGetForceUpdate;
    gGoppfForSet[E_HAL_QUERY_TRANSGOPIDTOTYPE]   = HalGopTransGopIdToType;
}
