...
void __HalPnlIfSetLpllConfigEx(DrvPnlCtxConfig_t *pPnlCtx, bool bLpMode)
{
    HalPnlHwLpllConfig_t *pstHwLpllCfg;
    HalPnlMipiDsiConfig_t *pstMipiDsiCfg;
...
void __HalPnlIfSetMipiDsiParamConfigEx(DrvPnlCtxConfig_t  *pPnlCtxCfg, HalPnlMipiDsiConfig_t *pstMipiDsiCfg)
{
    u32 u32Dclk;
    u16 u16Htotal, u16Vtotal;
    u8  au8PolCh[5];
    ...
    //Lp mode ?
    __HalPnlIfSetLpllConfigEx(pPnlCtxCfg, 1);
    ...
    // normal mode
    __HalPnlIfSetLpllConfigEx(pPnlCtxCfg, 0);
...
HalPnlQueryRet_e _HalPnlIfGetInfoUnifiedParamConfig(void *pCtx, void *pCfg)
{
    ...
    memcpy(&pPnlCtxCfg->pstHalUnfdCtx->stHalPnlUdParamCfg, pParamCfg, sizeof(HalPnlParamConfig_t));
...
void _HalPnlIfSetUnifiedParamConfig(void *pCtx, void *pCfg)
{
    ...
    {
        __HalPnlIfSetUnifiedPolarityConfig(pPnlCtxCfg, &pstUfdParamCfg->stTgnPolarityInfo);
    }

    //set swap channel
    if(pstUfdParamCfg->u8TgnRgbSwapFlag)
    {
        __HalPnlIfSetUnifiedSwapChnConfig(pPnlCtxCfg, &pstUfdParamCfg->stTgnRgbSwapInfo);
    }

    //set output bit mode
    if(pstUfdParamCfg->u8TgnOutputBitMdFlag)
    {
        if(DrvPnlOsPadMuxActive() == 0 && pstUfdParamCfg->eLinkType == E_HAL_PNL_LINK_TTL)
        {
            HalPnlSetTTLPadMux(pstUfdParamCfg->eOutputFormatBitMode);
        }

        HalPnlSetRgbMode(pstUfdParamCfg->eOutputFormatBitMode);// 565 or 888
    }
    ...
    {
        __HalPnlIfSetMipiDsiParamConfigEx(pPnlCtxCfg, &pstUfdParamCfg->stMpdInfo);
    }
