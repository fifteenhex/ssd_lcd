...
static void _SDMMC_REG_Dump(IPEmType eIP)
{
	...
	prtstring("\n----------------------------------------------------\r\n");
    prtU32Hex(((U32_T)A_SD_REG_POS(eIP) & 0x01FFFE00) >> 9);
	prtstring("  CMD_");
...
void Hal_SDMMC_TransCmdSetting(IPEmType eIP, TransEmType eTransType, U16_T u16BlkCnt, U16_T u16BlkSize, volatile U32_T u32BufAddr, volatile U8_T *pu8Buf)
{
	...
	if( (eTransType== EV_DMA) || (eTransType==EV_ADMA) )
	{
        ...
        else if ((u32BufAddr & 0xF0000000) == IMI_START_ADDR) {
            CARD_REG(A_DMA_ADDR_31_16_REG(eIP)) = (U16_T)((u32BufAddr-IMI_START_ADDR) >> 16);
        }
	}
	else //CIFD (R2N)
	{
...
void Hal_SDMMC_SDIOinterrupt(IPEmType eIP)
{
    U16_T u16reg = 0;

    //Hal_SDMMC_DumpMemTool(0x40, 0x1f282600);
    prtstring("\r\n A_SDIO_DET_ON:");
    prtU8Hex(CARD_REG(A_SDIO_DET_ON(eIP)));
    prtstring("\r\n A_MIE_INT_ENABLE_REG:");
    prtU8Hex(CARD_REG(A_MIE_INT_ENABLE_REG(eIP)));
    prtstring("\r\n A_SD_MODE_REG:");
    prtU8Hex(CARD_REG(A_SD_MODE_REG(eIP)));
    ...
    prtU8Hex(CARD_REG(A_SD_MODE_REG(eIP)));
    prtstring("\r\n A_MIE_EVENT_REG:");
    prtU8Hex(CARD_REG(A_MIE_EVENT_REG(eIP)));
