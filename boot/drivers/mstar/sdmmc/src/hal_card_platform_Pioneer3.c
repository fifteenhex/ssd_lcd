/*
* hal_card_platform_Pioneer3.c- Sigmastar
*
...
static volatile U32_T gu32_BondID = 0;
...
void Hal_CARD_IPOnceSetting(IPEmType eIP)
{
    ...
    if(eIP == EV_IP_FCIE1)
    {
        ...
        gu32_BondID = CARD_REG(GET_CARD_REG_ADDR(A_CHIPTOP_BANK, 0x48)) & BIT4; // 1:QFN128; 0:QFN68
    }
        ...
        /*
            ...
            */
            ...
            if (gu32_BondID)
                ...
                CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT09_T | BIT01_T); //[B8]/[reg_sdio_mode], [B0]/[reg_sdio_cdz_mode]
        }
    }
    else if(eIP == EV_IP_FCIE2)
    {
...
void Hal_CARD_SetClock(IPEmType eIP, U32_T u32ClkFromIPSet)
{
    ...
    #define FCIE1_CLK REG_CLK_SDIO0
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,FCIE1_CLK), BIT06_T|BIT05_T|BIT04_T|BIT03_T|BIT02_T|BIT01_T|BIT00_T); //[6:2]: Clk_Sel [1]: Clk_i [0]: Clk_g
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK,FCIE1_CLK), BIT06_T);//[6]: Sel fcie clk
...
void Hal_CARD_InitGPIO(GPIOEmType eGPIO, BOOL_T bEnable)
{
    if( eGPIO==EV_GPIO1 ) //EV_GPIO1 for Slot 0
    {
        ...
        if (gu32_BondID)
            ...
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PADTOP_BANK, 0x67), BIT01_T); //[B0]/[reg_sdio_cdz_mode]
        if(bEnable)
        {
        }
        else
...
BOOL_T Hal_CARD_GetGPIOState(GPIOEmType eGPIO)
{
    ...
    if( eGPIO==EV_GPIO1 ) //EV_GPIO1 for Slot 0
    {
        ...
        if (gu32_BondID)
            ...
            u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PADGPIO_BANK, 0x11)) & BIT00_T; //[0/1]:[insert/remove]
    }
