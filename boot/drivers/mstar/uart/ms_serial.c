...
//#define UART_RS485_GPIO (68)//if u need to use rs485,effect this code and change GPIO u control
//#define RS485_BEFORE_DELAY_US (1000)
//#define RS485_AFTER_DELAY_US (1000)
...
int ms_uart_getc (void)
{
    ...
    c=(char) ( UART_MULTI_REG8(UART_RX) & 0xff);
    return c;
}
...
#ifdef UART_RS485_GPIO
    mdrv_gpio_set_high(UART_RS485_GPIO);//add for rs485
    udelay(RS485_BEFORE_DELAY_US);
#endif
    if (c == '\n')
    {
...
#ifdef UART_RS485_GPIO
    udelay(RS485_AFTER_DELAY_US);
    mdrv_gpio_set_low(UART_RS485_GPIO);//add for rs485
#endif
}

#ifdef UART_RS485_GPIO
void ms_uart_rs485_config(void)
{
    //set gpio
    MDrv_GPIO_Pad_Set(UART_RS485_GPIO);
    MDrv_GPIO_Pad_Oen(UART_RS485_GPIO);
    MDrv_GPIO_Pull_Low(UART_RS485_GPIO);
    mdrv_gpio_set_low(UART_RS485_GPIO);//add for rs485
}
#endif

int ms_uart_lookback(const char c)
{
    printf("send data %x\r\n", c);
    while (!(UART_MULTI_REG8(UART_LSR) & UART_LSR_THRE));
    UART_MULTI_REG8(UART_TX) = c;
    ...
    while(!(UART_MULTI_REG8(UART_LSR) & UART_LSR_DR));
    char data =(char) ( UART_MULTI_REG8(UART_RX) & 0xff);
...
#ifdef UART_RS485_GPIO
    ms_uart_rs485_config();//add for rs485
#endif
