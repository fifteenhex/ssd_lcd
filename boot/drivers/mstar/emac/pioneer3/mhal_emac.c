...
#ifdef CONFIG_ETHERNET_LEDINVERT
    {
    u8 uRegVal;
    // INVERT LED1 & LED4
    uRegVal = MHal_EMAC_ReadReg8(REG_BANK_ALBANY0, 0xf7);
    uRegVal &= ~(0x0f);
    uRegVal |= 0x09;
    MHal_EMAC_WritReg8(REG_BANK_ALBANY0, 0xf7, uRegVal);

    // forece reg_led_ctrl = 1
    uRegVal = MHal_EMAC_ReadReg8(REG_BANK_ALBANY0, 0xfd);
    uRegVal &= ~(0x0f);
    uRegVal |= 0x02;
    MHal_EMAC_WritReg8(REG_BANK_ALBANY0, 0xfd, uRegVal);
    }
#endif
#ifdef CONFIG_ETHERNET_LEDREVERSE
    {
    u8 uRegVal;
    // reverse bit 15
    uRegVal = MHal_EMAC_ReadReg8(REG_BANK_ALBANY0, 0xf7);
...
#ifdef CONFIG_MS_QFN68
        // reg_eth0_mode; reg[103cdc]#3 ~ #0
