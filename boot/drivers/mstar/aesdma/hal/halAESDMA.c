...
void HAL_RSA_LoadSignInverse_2byte(U32 *ptr_Sign, U8 length)
{
    ...
    for( i = 0; i < length ; i++ )
    {
        RIU[(RSA_BASE_ADDR+(0x23<<1))]= (U16)(((*(ptr_Sign + (length-1) - i))>>8)&0xFF00)|(((*(ptr_Sign + (length-1) - i))>>24)&0xFF);
        HAL_RSA_DummyRead(0x23, 1);
        RIU[(RSA_BASE_ADDR+(0x24<<1))]= (U16)(((*(ptr_Sign + (length-1) - i))>>8)&0xFF)|(((*(ptr_Sign + (length-1) - i))<<8)&0xFF00);
        HAL_RSA_DummyRead(0x24, 1);
    }
...
void HAL_RSA_LoadKeyEInverse(U32 *ptr_E)
{
	...
	for( i = 0; i < length; i++ )
	{
		RIU[(RSA_BASE_ADDR+(0x23<<1))]= (U16)(((*(ptr_E + (length-1) - i))>>8)&0xFF00)|(((*(ptr_E + (length-1) - i))>>24)&0xFF);
        HAL_RSA_DummyRead(0x23, 1);
        RIU[(RSA_BASE_ADDR+(0x24<<1))]= (U16)(((*(ptr_E + (length-1) - i))>>8)&0xFF)|(((*(ptr_E + (length-1) - i))<<8)&0xFF00);
        HAL_RSA_DummyRead(0x24, 1);  
	}
	RIU[(RSA_BASE_ADDR+(0x20<<1))]= ((RIU[(RSA_BASE_ADDR+(0x20<<1))])&(~RSA_IND32_START)); //RSA stop
...
void HAL_RSA_LoadKeyNInverse(U32 *ptr_N)
{
    ...
    for( i = 0; i < length; i++ )
    {
        RIU[(RSA_BASE_ADDR+(0x23<<1))]= (U16)(((*(ptr_N + (length-1) - i))>>8)&0xFF00)|(((*(ptr_N + (length-1) - i))>>24)&0xFF);
        HAL_RSA_DummyRead(0x23, 1);
        RIU[(RSA_BASE_ADDR+(0x24<<1))]= (U16)(((*(ptr_N + (length-1) - i))>>8)&0xFF)|(((*(ptr_N + (length-1) - i))<<8)&0xFF00);
        HAL_RSA_DummyRead(0x24, 1);
    }
    RIU[(RSA_BASE_ADDR+(0x20<<1))]= ((RIU[(RSA_BASE_ADDR+(0x20<<1))])&(~RSA_IND32_START)); //RSA stop
}
