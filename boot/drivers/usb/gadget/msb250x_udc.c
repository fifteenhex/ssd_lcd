...
#ifdef CONFIG_SSTAR_UTMI_POWERON
    ms_writew(0x0C2F, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x04));
    ms_writew(0x040F, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x04));    //utmi0
    ms_writew(0x7F05, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x00));
    ms_writeb(0x00, GET_REG16_ADDR(USBC_BASE_ADDR, 0x29));
    //Module: utmi0 (0x142100)
    ms_writew(0x2084, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x01));    //Enable CLK12_SEL bit <2> for select low voltage crystal clock
    ms_writew(0x0426, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x04));    //bit<7>: Power down UTMI port-0 bandgap current

    ms_writew(0x6BC3, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x00));    //reg_pdn: bit<15>, bit <2> ref_pdn  # Turn on reference voltage and regulator
    ms_writeb(0x3F, GET_REG16_ADDR(USBC_BASE_ADDR, 0x29));      //overwrite off
    mdelay(1);
    ms_writew(0x69C3, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x00));    //Turn on UPLL, reg_pdn: bit<9>
    ms_writeb(0x3F, GET_REG16_ADDR(USBC_BASE_ADDR, 0x29));      //overwrite off
    mdelay(1);
    ms_writew(0x0001, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x00));    //Turn all (including hs_current) use override mode
    ms_writeb(0x00, GET_REG16_ADDR(USBC_BASE_ADDR, 0x29));      //overwrite off
    mdelay(3);
#else
