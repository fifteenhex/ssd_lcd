...
static int do_spi_flash_probe(int argc, char * const argv[])
{
	...
	if (argc >= 2) {
		if (cs != simple_strtoul(argv[1], &endp, 0))
		{
			address = simple_strtoul(argv[1], &endp, 0);
		}
...
static int do_spi_flash_read_write(int argc, char * const argv[])
{
	...
	if (*argv[1] == 0 || *endp != 0)
		return -1;
    if (arg_off_size(argc - 2, argv + 2, &idx, &offset, &size, &maxsize))
        return -1;
    ...
    if (strcmp(argv[0], "update") == 0) {
        ...
        printf("SF: %zu bytes @ %#x %s: %s", (size_t)len, (u32)offset,
...
static int do_spi_flash_erase(int argc, char * const argv[])
{
    ...
    if (arg_off_size(argc -1, argv + 1,&idx, (loff_t*)&offset, &size, &maxsize))
        return -1;
