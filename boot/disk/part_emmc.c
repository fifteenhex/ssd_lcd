...
static int part_emmc_read_ddb (block_dev_desc_t *dev_desc, emmc_driver_desc_t *ddb_p)
{
    ...
    if (dev_desc->block_read(dev_desc->dev, 0, 1, (uchar *)ddb_p) != 1) {
              printf ("** Can't read Driver Desriptor Block **\n");
        return (-1);
    }
...
//    }
	...
	return 0;
     ...
     int part=part_index+=1; //block of ppd start from 1
  ...
  if(dev_desc->block_read(dev_desc->dev, part, 1, (uchar *)pdb_p) != 1){
...
//                  "expected 0x%04x, got 0x%04x\n",
//                  dev_desc->dev, part_index, EMMC_PARTITION_MAGIC, pdb_p->signature);
...
static int part_emmc_write_ddb (block_dev_desc_t *dev_desc, emmc_driver_desc_t *ddb_p)
{
    ...
    if(dev_desc->block_write(dev_desc->dev, 0, 1, (uchar *)ddb_p) != 1){
...
static int part_emmc_write_pdb (block_dev_desc_t *dev_desc, int part_index, emmc_partition_t *pdb_p)
{
    ...
    if(dev_desc->block_write(dev_desc->dev, part, 1, (uchar *)pdb_p) != 1){
        printf("** Can't write Driver Desiptor Block **\n");
        return (-1);
    }
...
void print_part_emmc (block_dev_desc_t *dev_desc)
{
    ...
    if(part_emmc_read_ddb (dev_desc, &ddesc)) {
        printf("No valid partition table exist\n");
        return;
    }
...
//						return (0);
                    ...
                    if(dev_desc->block_write(dev_desc->dev, n, 1, (uchar *)pdb_p) != 1){
                        printf("** Can't write Driver Desiptor Block **\n");
                        return (-1);
                    }
                    return (0);
