...
UPGRADE_DIR=$TARGET_DIR/sd_upgrade
PADDED_BIN=$TARGET_DIR/padded.bin
PAD_DUMMY_BIN=$TARGET_DIR/dummy_pad
UPGRADE_FILE=$UPGRADE_DIR/SigmastarUpgradeSD.bin
...
CHIP_ID=`grep CONFIG_CHIP .config | awk -F '"' '{print $2}' `
IAMGE_CONFIG=`grep ^CONFIG_IMAGE_CONFIG .config | awk -F '"' '{print $2}' `
FLASH_TYPE=`grep ^FLASH_TYPE image/configs/$CHIP_ID/$IAMGE_CONFIG | awk -F '=' '{print $2}' | sed 's/ //g' `
UBOOT_IMAGE=`find image/output/images/boot -name u-boot*`
BOOT_SRC_DIR=board/$CHIP_ID/boot/sd_upgrade

SDMMC_CREATE_UPGRADE_DONE=0
    ...
    do
        if [ "$subContent" != "% <- this is end of file symbol" ] && [ "$subContent" != "sync_mmap" ]; then
            ...
            if [ "$subContent_prefix" == "estar" ]; then
                script_path=$(echo $subContent | awk '{print $2}')
                read -p "Upgrade $script_path? (y/N)" temp
                if [ "$temp" == "Y" ] || [ "$temp" == "y" ] || [ "$temp" == "" ]; then
                    func_process_sub_script $script_path
                fi
            ...
            elif [ "$subContent_prefix" == "fatformat" ]; then
                part_index=$(echo $subContent | awk '{print $3}' | awk -F ':' '{print $2}')
                if [ $part_index -gt 1 ]; then
                    echo $subContent >> $SCRIPT_FILE
                fi
            elif [ "$subContent_prefix" == "fdisk" ]; then
                fdisk_option=$(echo $subContent | awk '{print $2}')
                if [ "$fdisk_option" == "-c" ]; then
                    if [ $SDMMC_CREATE_UPGRADE_DONE -gt 0 ]; then
                        echo $subContent >> $SCRIPT_FILE
                    fi
                    SDMMC_CREATE_UPGRADE_DONE=$(($SDMMC_CREATE_UPGRADE_DONE+1))
                elif [ "$fdisk_option" == "-d" ]; then
                    part_index=$(echo $subContent | awk '{print $3}')
                    printf "fdisk -d.up %d\n" $part_index >> $SCRIPT_FILE
                else
                    echo $subContent >> $SCRIPT_FILE
                fi
            else
...
function func_init()
{
    ...
    rm -rf $UPGRADE_DIR
    mkdir -p $UPGRADE_DIR
    if [ $FLASH_TYPE == nor ] || [ $FLASH_TYPE == spinand ]; then
        cp $BOOT_SRC_DIR/IPL.$FLASH_TYPE.bin $UPGRADE_DIR/IPL
        cp $BOOT_SRC_DIR/IPL_CUST.$FLASH_TYPE.bin $UPGRADE_DIR/IPL_CUST
        cp $UBOOT_IMAGE $UPGRADE_DIR/UBOOT
    fi
    rm -f $SCRIPT_FILE
    rm -f $UPGRADE_FILE
