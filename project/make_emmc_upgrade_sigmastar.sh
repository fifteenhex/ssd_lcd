...
then
        echo
        echo "Set default 1 method for ugprade"
        upgrade_method=1
fi
    ...
    do
        if [ "$subContent" != "% <- this is end of file symbol" ] && [ "$subContent" != "sync_mmap" ]; then
            ...
            if [ "$subContent_prefix" == "tftp" ]; then
                ...
                if [ $upgrade_method -eq 1 ]; then
                    currentImageAddr_block=$(($currentImageOffset/512))
                    printf "emmc read.p.continue %s upgrade 0x%x 0x%x\n" $DRAM_BUF_ADDR $currentImageAddr_block $imageSize >> $SCRIPT_FILE
                else
        ...
        elif [ $upgrade_method -eq 1 ] && [ "$subContent_prefix" == "emmc" ]; then
            if [ $subContent == "emmc erase" ]; then
                echo "emmc rmgpt" >> $SCRIPT_FILE
            elif [[ $subContent == "emmc create upgrade"* ]]; then
                echo "No need to create upgrade partition"
            else
