...
rootfs:
	...
	if [ "$(LINUX_FLASH_TYPE)"x = "emmc"x  ] || [ "$(FLASH_TYPE)"x = "sdmmc"x  ]; then \
		   echo mdev -s >> $(OUTPUTDIR)/rootfs/etc/init.d/rcS ;\
	...
	if [ "$(FWFS_MOUNT_BLK)" != "" ]; then \
		...
		mkdir -p $(OUTPUTDIR)/rootfs/misc;\
		$(foreach block, $(FWFS_MOUNT_BLK), $(PROJ_ROOT)/image/makefiletools/script/fwfs_mount.py --linux_flash_type=$(LINUX_FLASH_TYPE) --flash_type=$(FLASH_TYPE) --block_size=$(FLASH_BLK_SIZE) --page_size=$(FLASH_PG_SIZE) --part_size=$($(block)$(PATSIZE)) --rcs_dir=$(OUTPUTDIR)/rootfs/etc/init.d/rcS $($(block)$(MOUNTPT)) $($(block)$(MOUNTTG));)\
	fi;
...
#	echo "busybox telnetd&" >> $(OUTPUTDIR)/rootfs/etc/init.d/rcS
	...
	if [ "$(ramdisk$(RESOUCE))" != "" ]; then \
		...
		echo "mount -t ubifs ubi0:customer /customer" >> $(OUTPUTDIR)/tmprd/etc/init.d/rcS; \
		    ...
		    echo -e mdev -s >> $(OUTPUTDIR)/tmprd/etc/init.d/rcS;	\
		    $(foreach block, $(FWFS_MOUNT_BLK), $(PROJ_ROOT)/image/makefiletools/script/fwfs_mount.py --linux_flash_type=$(LINUX_FLASH_TYPE) --flash_type=$(FLASH_TYPE) --block_size=$(FLASH_BLK_SIZE) --page_size=$(FLASH_PG_SIZE) --part_size=$($(block)$(PATSIZE)) --rcs_dir=$(OUTPUTDIR)/tmprd/etc/init.d/rcS $($(block)$(MOUNTPT)) $($(block)$(MOUNTTG));)\
	        fi; \
