...
rootfs:
	...
	if [ "$(verify_mi_demo)" = "enable" ]; then \
		if [ -f "$(LIB_DIR_PATH)/bin/mi_demo/prog_uvc" ]; then \
			cp -rf $(LIB_DIR_PATH)/bin/mi_demo/prog_uvc $(OUTPUTDIR)/customer/ ; \
		fi; \
	fi;
...
ifeq ($(verify_uvc_uac), disable)
	...
	if [ -f "$(KBUILD_CUST)/kernel_mod_list_dual_uvc" ]; then \
		cat $(KBUILD_CUST)/kernel_mod_list_dual_uvc | sed 's#\(.*\).ko#insmod /config/modules/$(KERNEL_VERSION)/\1.ko#' >> $(OUTPUTDIR)/customer/demo.sh; \
		cat $(KBUILD_CUST)/kernel_mod_list_dual_uvc | sed 's#\(.*\).ko\(.*\)#$(KERN_MODS_PATH)/\1.ko#' | xargs -i cp -rvf {} $(miservice$(RESOUCE))/modules/$(KERNEL_VERSION); \
		echo "#kernel_mod_list_dual_uvc" >> $(OUTPUTDIR)/customer/demo.sh; \
	fi;
    endif
else ifeq ($(verify_uvc_uac), enable)
    ifndef SENSOR1
	if [ -f "$(KBUILD_CUST)/kernel_mod_list_uvc_uac" ]; then \
		cat $(KBUILD_CUST)/kernel_mod_list_uvc_uac | sed 's#\(.*\).ko#insmod /config/modules/$(KERNEL_VERSION)/\1.ko#' >> $(OUTPUTDIR)/customer/demo.sh; \
		cat $(KBUILD_CUST)/kernel_mod_list_uvc_uac | sed 's#\(.*\).ko\(.*\)#$(KERN_MODS_PATH)/\1.ko#' | xargs -i cp -rvf {} $(miservice$(RESOUCE))/modules/$(KERNEL_VERSION); \
		echo "#kernel_mod_list_uvc_uac" >> $(OUTPUTDIR)/customer/demo.sh; \
	fi;
    else
	if [ -f "$(KBUILD_CUST)/kernel_mod_list_dual_uvc_uac" ]; then \
		cat $(KBUILD_CUST)/kernel_mod_list_dual_uvc_uac | sed 's#\(.*\).ko#insmod /config/modules/$(KERNEL_VERSION)/\1.ko#' >> $(OUTPUTDIR)/customer/demo.sh; \
		cat $(KBUILD_CUST)/kernel_mod_list_dual_uvc_uac | sed 's#\(.*\).ko\(.*\)#$(KERN_MODS_PATH)/\1.ko#' | xargs -i cp -rvf {} $(miservice$(RESOUCE))/modules/$(KERNEL_VERSION); \
		echo "#kernel_mod_list_dual_uvc_uac" >> $(OUTPUTDIR)/customer/demo.sh; \
	fi;
    endif
endif
	if [ "$(DUAL_OS)" != "on" ]; then \
...
#	echo "busybox telnetd&" >> $(OUTPUTDIR)/rootfs/etc/init.d/rcS
	...
	if [ "$(ramdisk$(RESOUCE))" != "" ]; then \
		...
		echo "mount -t ubifs ubi0:customer /customer" >> $(OUTPUTDIR)/tmprd/etc/init.d/rcS; \
		    ...
		    echo -e mdev -s >> $(OUTPUTDIR)/tmprd/etc/init.d/rcS;	\
		    $(foreach block, $(FWFS_MOUNT_BLK), $(PROJ_ROOT)/image/makefiletools/script/fwfs_mount.py --flash_type=$(FLASH_TYPE) --block_size=$(FLASH_BLK_SIZE) --page_size=$(FLASH_PG_SIZE) --part_size=$($(block)$(PATSIZE)) --rcs_dir=$(OUTPUTDIR)/tmprd/etc/init.d/rcS $($(block)$(MOUNTPT)) $($(block)$(MOUNTTG));)\
	        fi; \
