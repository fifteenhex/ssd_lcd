...
## ramdisk/other use /linuxrc , ramfs use /init
	...
	$(Q) mkdir -p $(OUTPUTDIR)/rootfs/customer_app
	...
	$(Q) mkdir -p $(OUTPUTDIR)/rootfs/customer_app/iqfile/
	$(Q) cp -rf $(PROJ_ROOT)/board/$(CHIP)/iqfile/isp_api.xml $(OUTPUTDIR)/rootfs/customer_app/iqfile/ -f
	$(Q) if [ "$(IQ0)" != "" ]; then \
	    	cp -rf $(PROJ_ROOT)/board/$(CHIP)/iqfile/$(IQ0) $(OUTPUTDIR)/rootfs/customer_app/iqfile/ -f; \
	 		cd $(OUTPUTDIR)/rootfs/customer_app/iqfile; chmod +x $(shell echo $(IQ0) | awk -F'/' '{print $$NF}'); ln -sf $(shell echo $(IQ0) | awk -F'/' '{print $$NF}') iqfile0.bin; cd -; \
	 	 fi;
	$(Q) if [ "$(IQ1)" != "" ]; then \
		 	cp -rf $(PROJ_ROOT)/board/$(CHIP)/iqfile/$(IQ1) $(OUTPUTDIR)/rootfs/customer_app/iqfile/ -f; \
		 	cd $(OUTPUTDIR)/rootfs/customer_app/iqfile; chmod +x $(shell echo $(IQ1) | awk -F'/' '{print $$NF}'); ln -sf $(shell echo $(IQ1) | awk -F'/' '{print $$NF}') iqfile1.bin; cd -; \
		 fi;
	$(Q) if [ "$(IQ2)" != "" ]; then \
		 	cp -rf $(PROJ_ROOT)/board/$(CHIP)/iqfile/$(IQ2) $(OUTPUTDIR)/rootfs/customer_app/iqfile/ -f; \
		 	cd $(OUTPUTDIR)/rootfs/customer_app/iqfile; chmod +x $(shell echo $(IQ2) | awk -F'/' '{print $$NF}'); ln -sf $(shell echo $(IQ2) | awk -F'/' '{print $$NF}') iqfile2.bin; cd -; \
		 fi;
	$(Q) if [ "$(IQ3)" != "" ]; then \
		 	cp -rf $(PROJ_ROOT)/board/$(CHIP)/iqfile/$(IQ3) $(OUTPUTDIR)/rootfs/customer_app/iqfile/ -f; \
		 	cd $(OUTPUTDIR)/rootfs/customer_app/iqfile; chmod +x $(shell echo $(IQ3) | awk -F'/' '{print $$NF}'); ln -sf $(shell echo $(IQ3) | awk -F'/' '{print $$NF}') iqfile3.bin; cd -; \
		 fi;
	$(Q) if [ "$(IQ_API0)" != "" ]; then \
		 	cp -rf $(PROJ_ROOT)/board/$(CHIP)/iqfile/$(IQ_API0) $(OUTPUTDIR)/rootfs/customer_app/iqfile/ -f; \
		 	cd $(OUTPUTDIR)/rootfs/customer_app/iqfile; chmod +x $(shell echo $(IQ_API0) | awk -F'/' '{print $$NF}'); ln -sf $(shell echo $(IQ_API0) | awk -F'/' '{print $$NF}') iqapifile0.bin; cd -; \
		 fi;
	$(Q) if [ "$(IQ_API1)" != "" ]; then \
		 	cp -rf $(PROJ_ROOT)/board/$(CHIP)/iqfile/$(IQ_API1) $(OUTPUTDIR)/rootfs/customer_app/iqfile/ -f; \
		 	cd $(OUTPUTDIR)/rootfs/customer_app/iqfile; chmod +x $(shell echo $(IQ_API1) | awk -F'/' '{print $$NF}'); ln -sf $(shell echo $(IQ_API1) | awk -F'/' '{print $$NF}') iqapifile1.bin; cd -; \
		 fi;

	$(Q) cp -rdf $(RELEASE_ROOT)/bin/mi_demo/prog_uvc $(OUTPUTDIR)/rootfs/customer_app/
	$(Q) $(STRIP)  --strip-unneeded $(OUTPUTDIR)/rootfs/customer_app/prog_uvc
	$(Q) chmod +x $(OUTPUTDIR)/rootfs/customer_app/prog_uvc
...
rootfs_fastboot_$(PRODUCT_NEAT)_miservice:rootfs_fastboot_$(PRODUCT_NEAT)_customer
	...
	if [ -f "$(LIB_DIR_PATH)/bin/debug/riu_r" ]; then \
		-chmod 755 $(LIB_DIR_PATH)/bin/debug/riux32_r  ;\
		cp -rf $(LIB_DIR_PATH)/bin/debug/riux32_r $(OUTPUTDIR)/customer/   ;\
	fi;	\
	if [ -f "$(LIB_DIR_PATH)/bin/debug/riu_w" ]; then \
		-chmod 755 $(LIB_DIR_PATH)/bin/debug/riux32_w  ;\
		cp -rf $(LIB_DIR_PATH)/bin/debug/riux32_w $(OUTPUTDIR)/customer/   ;\
	fi;
...
rootfs_fastboot_$(PRODUCT_NEAT): rootfs_fastboot_$(PRODUCT_NEAT)_done
	$(Q) if [ "$(DUAL_OS)" != "on" ]; then \
			if [ "$(SENSOR_LIST)" != "" ]; then \
				cp -rf $(foreach n,$(SENSOR_LIST),$(LIB_DIR_PATH)/modules/$(KERNEL_VERSION)/$(n)) $(OUTPUTDIR)/rootfs/lib/modules/$(KERNEL_VERSION); \
			fi; \
			if [ "$(SENSOR0)" != "" ]; then \
	...
	$(Q) echo "export LD_LIBRARY_PATH=\$$LD_LIBRARY_PATH:/usr/local/lib:/customer_app/lib:/lib:/config/lib:/config/wifi" >> $(INIT_FILE)
	$(Q) echo "echo /customer_app/iqfile > /sys/devices/virtual/mstar/ispmid0/isproot" >> $(INIT_FILE)
	$(Q) echo "cd /customer_app" >> $(INIT_FILE)
	$(Q) echo "chmod +x prog_uvc" >> $(INIT_FILE)
	$(Q) echo "./prog_uvc -A 0 &" >> $(INIT_FILE)
