...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(RN6752M);
...
#define BIND_SENSOR_RN6752M (1)
#endif //#if defined(__RTK_OS__)

#if defined(BIND_SENSOR_RN6752M) && (BIND_SENSOR_RN6752M == 1)
...
#define Preview_WIDTH_FHD      1920                   //resolution Width when preview
#define Preview_HEIGHT_FHD     1080                   //resolution Height when preview

#define Preview_WIDTH_HD       1280                   //resolution Width when preview
#define Preview_HEIGHT_HD      720                    //resolution Height when preview

#define Preview_WIDTH_CVBS     640
#define Preview_HEIGHT_CVBS    480
...
#define SENSOR_I2C_CHANNEL 	2							//I2C Channel
...
typedef enum {
    RN6752M_ANADEC_SRC_NO_READY=0,
    RN6752M_CVBS_PAL,
    RN6752M_CVBS_NTSC,
	RN6752M_AHD_1080P_25FPS ,
	RN6752M_AHD_1080P_30FPS ,
	RN6752M_AHD_720P_25FPS  ,
	RN6752M_AHD_720P_30FPS	,
    RN6752M_AHD_CVBS_25FPS,
    RN6752M_AHD_CVBS_30FPS,
    RN6752M_AHD_MAX,
}RN6752M_Type;

char   ui32RN6752MType = RN6752M_AHD_720P_25FPS;

static u8 sensor_update_flag;

static u8 old_psrc_type=0;

static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain);
static int pCus_SetAEUSecs(ms_cus_sensor *handle, u32 us);
int rn6752m_cus_camsensor_release_handle(ms_cus_sensor *handle);
#if 0
...
static I2C_ARRAY Sensor_init_table_FHD_25FPS[] =
{
	// 1080P@25 BT656
	// Slave address is 0x58
	// Register, data
...
static I2C_ARRAY Sensor_init_table_FHD_30FPS[] =
{
	// 1080P@30 BT656
	// Slave address is 0x58
	// Register, data
...
static I2C_ARRAY Sensor_init_table_HD_25FPS[] =
{
...
static I2C_ARRAY Sensor_init_table_HD_30FPS[] =
{
...
/*
void Check_Sensor_Type(ms_cus_sensor *handle)
{
    unsigned short  count_bit4 = 0, check_sig = 0, check_bit4 = 0;
    unsigned short rn6752m_reg_00;
    unsigned int i;
    SensorReg_Write(0x19,0x07);
    for (i=0; i<100; i++ ) 
    {       
        SENSOR_MSLEEP(1);
        SensorReg_Read(0x00,&check_bit4);
        check_bit4=check_bit4>>4;
    ...
    SENSOR_MSLEEP(10);
    SENSOR_EMSG("EEEEEEEEEcheck_sig=%d\n",check_sig);
    if(check_sig)
    {
        SensorReg_Read(0x00,&rn6752m_reg_00);
        rn6752m_reg_00=rn6752m_reg_00&0xf1; 
        switch (rn6752m_reg_00)
        {
            case 0x00:
                SENSOR_DMSG("####RN6752M_DecoderLock signal CVBS PAL\r\n");
                ui32RN6752MType=RN6752M_AHD_CVBS_25FPS;
                break;
            case 0x01:
                SENSOR_DMSG("####RN6752M_DecoderLock signal CVBS NTSC\r\n");
                ui32RN6752MType=RN6752M_AHD_CVBS_30FPS;
                break;
            case 0x20:
                SENSOR_DMSG("###RN6752M_DecoderLock signal 720P PAL\r\n");
                ui32RN6752MType=RN6752M_AHD_720P_25FPS;
                break;
            case 0x21:
                SENSOR_DMSG("####RN6752M_DecoderLock signal 720P NTSC\r\n");
                ui32RN6752MType=RN6752M_AHD_720P_30FPS;
                break;
            case 0x40:
                SENSOR_DMSG("####RN6752M_DecoderLock signal 1080P PAL\r\n");
                ui32RN6752MType=RN6752M_AHD_1080P_25FPS;
                break;
            case 0x41:
                SENSOR_DMSG("####RN6752M_DecoderLock signal 1080P NTSC\r\n");
                ui32RN6752MType=RN6752M_AHD_1080P_30FPS;
                break;
            default:
                ui32RN6752MType=RN6752M_AHD_720P_25FPS;
                SENSOR_DMSG("###############unknow type\n");
                break;
        }
        
    }
    switch(ui32RN6752MType)
    {
        case RN6752M_AHD_CVBS_25FPS:
    	case RN6752M_AHD_CVBS_30FPS:
    	{
			handle->video_res_supported.res[0].width = Preview_WIDTH_CVBS;
			handle->video_res_supported.res[0].height = Preview_HEIGHT_CVBS;
			handle->video_res_supported.res[0].nOutputWidth= Preview_WIDTH_CVBS;
			handle->video_res_supported.res[0].nOutputHeight= Preview_HEIGHT_CVBS;
    	}
    	break;
        case RN6752M_AHD_720P_25FPS:
    	case RN6752M_AHD_720P_30FPS:
    	{
			handle->video_res_supported.res[0].width = Preview_WIDTH_HD;
			handle->video_res_supported.res[0].height = Preview_HEIGHT_HD;
			handle->video_res_supported.res[0].nOutputWidth= Preview_WIDTH_HD;
			handle->video_res_supported.res[0].nOutputHeight= Preview_HEIGHT_HD;
    	}
    	break;
        case RN6752M_AHD_1080P_25FPS:
    	case RN6752M_AHD_1080P_30FPS:
    	{
			handle->video_res_supported.res[0].width = Preview_WIDTH_FHD;
			handle->video_res_supported.res[0].height = Preview_HEIGHT_FHD;
			handle->video_res_supported.res[0].nOutputWidth= Preview_WIDTH_FHD;
			handle->video_res_supported.res[0].nOutputHeight= Preview_HEIGHT_FHD;
    	}
    	break;
    }
}
...
static int rn6752m_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
#if 0
	int i;
    	SENSOR_DMSG("[%s]\n", __FUNCTION__);
...
static int pCus_init(ms_cus_sensor *handle)
{
   ...
   // unsigned short sen_data;
    ...
    #if 1
   {
		unsigned short rom_byte1, rom_byte2, rom_byte3, rom_byte4, rom_byte5, rom_byte6;
    ...
    SENSOR_EMSG("#########ui32RN6752MType=%d\n",ui32RN6752MType);
    switch(ui32RN6752MType)
	{
		case RN6752M_AHD_1080P_30FPS:
		for (i = 0; i < ARRAY_SIZE(Sensor_init_table_FHD_30FPS); i++)
		{
			if (Sensor_init_table_FHD_30FPS[i].reg==0xffff)
			{
				SENSOR_MSLEEP(1);//usleep(1000*Sensor_init_table[i].data);
			}
			else
			{
				cnt = 0;
	
				while (SensorReg_Write(Sensor_init_table_FHD_30FPS[i].reg, Sensor_init_table_FHD_30FPS[i].data) != SUCCESS)
				{
					cnt++;
				...
				UartSendTrace("RN6752M_BT656 reg: 0x%x, data: 0x%x, read: 0x%x.\n",Sensor_init_table[i].reg, Sensor_init_table[i].data, sen_data);
				#endif
			}
		}
		break;
		case RN6752M_AHD_1080P_25FPS:
		SENSOR_EMSG("###########init 1080p 25fps\n");
		for (i = 0; i < ARRAY_SIZE(Sensor_init_table_FHD_25FPS); i++)
		{
			if (Sensor_init_table_FHD_25FPS[i].reg==0xffff)
			{
				SENSOR_MSLEEP(1);
			}
			else
			{
				...
				while (SensorReg_Write(Sensor_init_table_FHD_25FPS[i].reg, Sensor_init_table_FHD_25FPS[i].data) != SUCCESS)
				{
					cnt++;
					SENSOR_DMSG("[%s] set Sensor_init_table retry : %d...\n",__FUNCTION__,cnt);
		...
		break;
		case RN6752M_AHD_720P_25FPS:
		for (i = 0; i < ARRAY_SIZE(Sensor_init_table_HD_25FPS); i++)
		{
			if (Sensor_init_table_HD_25FPS[i].reg==0xffff)
			{
				SENSOR_MSLEEP(1);
			}
			else
			{
				...
				while (SensorReg_Write(Sensor_init_table_HD_25FPS[i].reg, Sensor_init_table_HD_25FPS[i].data) != SUCCESS)
				{
					cnt++;
					SENSOR_DMSG("[%s] set Sensor_init_table retry : %d...\n",__FUNCTION__,cnt);
		...
		break;
		case RN6752M_AHD_720P_30FPS:
		for (i = 0; i < ARRAY_SIZE(Sensor_init_table_HD_30FPS); i++)
		{
			if (Sensor_init_table_HD_30FPS[i].reg==0xffff)
			{
				SENSOR_MSLEEP(1);
			}
			else
			{
				...
				while (SensorReg_Write(Sensor_init_table_HD_30FPS[i].reg, Sensor_init_table_HD_25FPS[i].data) != SUCCESS)
				{
					cnt++;
					SENSOR_DMSG("[%s] set Sensor_init_table retry : %d...\n",__FUNCTION__,cnt);
...
#if 0
	char index=0;
	rn6752m_params *params = (rn6752m_params *)handle->private_data;
...
static int rn6752m_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
	SENSOR_DMSG("[%s]\n", __FUNCTION__);
...
static int pCus_BT656_GetSrcType(ms_cus_sensor *handle, u32 plane_id, CUS_SNR_ANADEC_SRC_TYPE *psrc_type)
{
    ...
    *psrc_type = CUS_SNR_ANADEC_SRC_NO_READY;
    ...
    if(usCnntStatus == 0x45 || usCnntStatus == 0x44) {
        if(RN6752M_AHD_1080P_25FPS == ui32RN6752MType)
            *psrc_type = CUS_SNR_ANADEC_SRC_FHD_25P;
        if(RN6752M_AHD_1080P_30FPS == ui32RN6752MType)
            *psrc_type = CUS_SNR_ANADEC_SRC_FHD_30P;
    } else if(usCnntStatus == 0x25 || usCnntStatus == 0x24) {
        if(RN6752M_AHD_720P_25FPS == ui32RN6752MType)
            *psrc_type = CUS_SNR_ANADEC_SRC_HD_25P;
        if(RN6752M_AHD_720P_30FPS == ui32RN6752MType)
...
#endif
int rn6752m_cus_camsensor_init_handle(ms_cus_sensor* handle)
{
    rn6752m_params *params;
    if (!handle)
    {
    ...
    params = (rn6752m_params *)handle->private_data;
    ...
    handle->video_res_supported.ulcur_res = 0; //default resolution index is 0.
     if((RN6752M_AHD_1080P_25FPS == ui32RN6752MType) || (RN6752M_AHD_1080P_30FPS == ui32RN6752MType))
    {
		handle->video_res_supported.res[0].width = Preview_WIDTH_FHD;
		handle->video_res_supported.res[0].height = Preview_HEIGHT_FHD;
		handle->video_res_supported.res[0].nOutputWidth= Preview_WIDTH_FHD;
    ...
    SENSOR_EMSG("#######inw=%d,inh=%d\n",handle->video_res_supported.res[0].width,handle->video_res_supported.res[0].height);
    SENSOR_EMSG("#######outw=%d,outh=%d\n",handle->video_res_supported.res[0].nOutputWidth,handle->video_res_supported.res[0].nOutputHeight);
    ...
    handle->pCus_sensor_release     = rn6752m_cus_camsensor_release_handle;
    ...
    handle->pCus_sensor_SetPatternMode = rn6752m_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo   = rn6752m_GetShutterInfo;
...
#if defined(__RTK_OS__)
struct SensorMapTable_t gsensor_map_rn6752m[] = {
    { //Channel 0
        SENSOR_CHANNEL_NUM,
        rn6752m_cus_camsensor_init_handle,
        rn6752m_cus_camsensor_release_handle
    },
};

int RN6752m_Module_Init(void)
{
    int iloop = 0;
    for(iloop = 0; iloop < SENSOR_BT656_CH_TOTAL_NUM; ++iloop){
        cameraSensorRegister(&gsensor_map_rn6752m[iloop]);
    }
    return 0;
}

void RN6752m_Module_Exit(void)
{
    int iloop = 0;
    for(iloop = SENSOR_BT656_CH_TOTAL_NUM; iloop >= 0; --iloop){
        cameraSensorUnRegister(&gsensor_map_rn6752m[iloop]);
    }
}
...
#endif
ait_module_init(RN6752m_Module_Init);
//ait_module_exit(RN6752_Module_Exit);
#endif //#if defined(__RTK_OS__)

SENSOR_DRV_ENTRY_IMPL_END_EX(  RN6752m,
                            rn6752m_cus_camsensor_init_handle,
                            NULL,
                            NULL,
                            rn6752m_params
