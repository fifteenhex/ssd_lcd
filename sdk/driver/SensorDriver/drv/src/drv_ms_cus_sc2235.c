...
{0x3313,0x05},
{0x3678,0x42},
{0x3677,0x3f}, //az margin
{0x3306,0x44},//20180125[3c,4f]
{0x330b,0xcb},//20180125[c3,d5]
{0x3039,0x31},

//20180202
{0x3237,0x08},
{0x3238,0x68}, //hts-0x30
...
static int sc2235SetAEUSecs(ms_cus_sensor *handle, u32 us)
{
    ...
    lines=(1000*us + (Preview_line_period >> 1))/Preview_line_period + 1; // Preview_line_period in ns
    if(lines<1) lines=1;
...
static int sc2235SetAEGain(ms_cus_sensor *handle, u32 gain)
{
    ...
    Fine_gain_reg = abs(gain / (Dgain * Coarse_gain * 64));
    //Fine_gain_reg = abs(((Fine_gain - 10000) * 16)/10000) + 16;
    ...
    //pr_info("[%s] set gain =%d 0x%x ,0x%x, 0x%x\n", __FUNCTION__, gain,gain_reg[0].data,gain_reg[1].data,gain_reg[2].data);
#if ENABLE_NR
    if (gain < 2*1024) {
        params->tNr_reg[0].data = 0x12;  //0x05;
        params->tNr_reg[1].data = 0x84;
        params->tNr_reg[2].data = 0x2f;
        params->tNr_reg[3].data = 0xc6;
    } else if (gain < 8*1024) {
        params->tNr_reg[0].data = 0x15;  //0x13;
        params->tNr_reg[1].data = 0x88;
        params->tNr_reg[2].data = 0x2f;
        params->tNr_reg[3].data = 0xc6;
    } else if (Coarse_gain_reg == 7 && Fine_gain_reg < 0x1f) {
        params->tNr_reg[0].data = 0x1c;  //0x14;
        params->tNr_reg[1].data = 0x88;
        params->tNr_reg[2].data = 0x2f;
        params->tNr_reg[3].data = 0xc6;
    } else if (Coarse_gain_reg == 7 && Fine_gain_reg == 0x1f) {
        params->tNr_reg[0].data = 0xff;
        params->tNr_reg[1].data = 0x88;
        params->tNr_reg[2].data = 0x2f;//3a
        params->tNr_reg[3].data = 0xc6;//06
    }
