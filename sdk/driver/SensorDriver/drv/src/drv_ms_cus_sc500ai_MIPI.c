...
I2C_ARRAY Sensor_init_table_5M30fps[] =
{
    {0x0103,0x01},
    {0x0100,0x00},
    {0x36e9,0x80},
...
static int sc500ai_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
  int i;
  SENSOR_DMSG("\n\n[%s], mode=%d \n", __FUNCTION__, mode);
...
static int pCus_init_linear_5M30fps(ms_cus_sensor *handle)
{
    ...
    {
        if(Sensor_init_table_5M30fps[i].reg==0x0100 && 0x01 == Sensor_init_table_5M30fps[i].data)
        {
...
//  SENSOR_DMSG("[%s] us %ld, half_lines %ld, vts %ld\n", __FUNCTION__, us, half_lines, params->expo.vts);
 ...
 //   pr_info("[%s]  expo_reg_HDR_SEF : %x , %x\n\n", __FUNCTION__,expo_reg_HDR_SEF[0].data,expo_reg_HDR_SEF[1].data);
...
static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain) {
    ...
    u32 Fine_againx64 = 32,Fine_dgainx128 = 128;
    u8 Dgain_reg = 0, Coarse_gain_reg = 0, Fine_again_reg= 0x40,Fine_dgain_reg= 0x80;
    ...
    else if (gain <  127 * 1024)
    {
        ...
        Dgain_reg = 0x3;  Fine_again_reg = 0x7f;  Coarse_gain_reg = 0x1f;
    }
    else if (gain <  254 * 1024)
    {
        Dgain = 8;      Fine_againx64 = 127;    Coarse_gain = 8;
        Dgain_reg = 0x7;  Fine_again_reg = 0x7f;  Coarse_gain_reg = 0x1f;
    }
    else if (gain <=(SENSOR_MAXGAIN * 1024))
    {
    ...
    if (gain <=  16256)
    {
        Fine_again_reg = abs(8 * gain / (Dgain * Coarse_gain * Fine_dgainx128));
    }
    else
    {
...
static int pCus_SetAEGain_HDR_SEF(ms_cus_sensor *handle, u32 gain) {
    ...
    if (gain < 2*1024) {
        ...
        params->tNr_reg[1].data = 0x06;
        params->tNr_reg[2].data = 0x64;
        params->tNr_reg[3].data = 0x82;
        params->tNr_reg[4].data = 0x33;
    } else if (gain < 4*1024) {
        params->tNr_reg[0].data = 0x05;
        params->tNr_reg[1].data = 0x16;
        params->tNr_reg[2].data = 0x64;
        params->tNr_reg[3].data = 0x62;
        params->tNr_reg[4].data = 0x44;
    }else if (gain < 8*1024) {
        params->tNr_reg[0].data = 0x08;
        params->tNr_reg[1].data = 0x16;
        params->tNr_reg[2].data = 0x64;
        params->tNr_reg[3].data = 0x44;
        params->tNr_reg[4].data = 0x44;
    } else {
        params->tNr_reg[0].data = 0x10;
        params->tNr_reg[1].data = 0x17;
        params->tNr_reg[2].data = 0x64;
        params->tNr_reg[3].data = 0x44;
        params->tNr_reg[4].data = 0x66;
    }
#endif
   ...
   // printk("[%s]  gain_reg : %x ,%x ,%x , %x\n\n", __FUNCTION__,Fine_again_reg,Coarse_gain_reg,Fine_dgain_reg,Dgain_reg);
    ...
    params->tGain_reg_HDR_SEF[3].data = Fine_again_reg;
    params->tGain_reg_HDR_SEF[2].data = Coarse_gain_reg;
    params->tGain_reg_HDR_SEF[1].data = Fine_dgain_reg;
    params->tGain_reg_HDR_SEF[0].data = Dgain_reg;
...
static int sc500ai_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int sc500ai_GetShutterInfo_HDR_LEF(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int sc500ai_GetShutterInfo_HDR_SEF(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    sc500ai_params *params = (sc500ai_params *)handle->private_data;
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    SENSOR_DMSG(handle->model_id,"sc500ai_MIPI");
    ...
    handle->video_res_supported.res[0].nOutputWidth= 2880;
    handle->video_res_supported.res[0].nOutputHeight= 1620;
    ...
    handle->pCus_sensor_GetShutterInfo = sc500ai_GetShutterInfo;
    params->expo.vts=vts_30fps;
    params->expo.fps = 30;
...
{
    ms_cus_sensor *handle = drv_handle;
    sc500ai_params *params = NULL;

    cus_camsensor_init_handle(drv_handle);
    params = (sc500ai_params *)handle->private_data;

    sprintf(handle->model_id,"sc500ai_MIPI_HDR_SEF");
    ...
    handle->video_res_supported.res[0].nOutputWidth = 2880;
    handle->video_res_supported.res[0].nOutputHeight = 1620;
    sprintf(handle->video_res_supported.res[0].strResDesc, "2880x1620@30fps_HDR");
    ...
    handle->pCus_sensor_GetShutterInfo = sc500ai_GetShutterInfo_HDR_SEF;
...
int cus_camsensor_init_handle_HDR_LEF(ms_cus_sensor* drv_handle) {
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->model_id,"sc500ai_MIPI_HDR_LEF");
    ...
    handle->pCus_sensor_GetShutterInfo = sc500ai_GetShutterInfo_HDR_LEF;
    handle->pCus_sensor_CustDefineFunction = pCus_sensor_CustDefineFunction;
