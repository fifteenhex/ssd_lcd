...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(OV2710);
...
#define Preview_line_period 30250                  //
...
const I2C_ARRAY Sensor_init_table[] =     // 1920*1080_30fps_27MCLK_756MMIPI_2840*1109*30
{
    ...
    {0x5686,0x04},
    {0x5687,0x43},
    {0x3011,0x14},//;for MCLK=12MHz 0x14, default 24MHz is 0a
    {0x300f,0x8a},
    {0x3017,0x00},
    {0x3018,0x00},
    {0x300e,0x04},
    ...
    *id =0;
    ...
    for(i=0; i<table_length; ++i) {
        if( id_from_sensor[i].data != Sensor_id_table[i].data ) {
            SENSOR_EMSG("[%s]Read OV2710 id: 0x%x 0x%x\n", __FUNCTION__, id_from_sensor[0].data, id_from_sensor[1].data);
            return FAIL;
        }
        *id = id_from_sensor[i].data;
    }
    SENSOR_EMSG("[%s]Read OV2710 id, get 0x%x Success\n", __FUNCTION__, (int)*id);
    return SUCCESS;
}
...
static int pCus_post_init(ms_cus_sensor *handle, u32 idx)
{
    ...
    SENSOR_USLEEP(10000);  
    //SensorReg_Write(0xffff, Sensor_init_table[i].data)
    SensorReg_Write(0x3008, 0x02);
    return SUCCESS;
}
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
    ...
    params = (OV2710_params *)handle->private_data;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    strcpy(handle->model_id,"OV2710_MIPI");
...
SENSOR_DRV_ENTRY_IMPL_END_EX(  OV2710,
