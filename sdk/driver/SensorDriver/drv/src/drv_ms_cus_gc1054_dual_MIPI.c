...
SENSOR_DRV_ENTRY_IMPL_BEGIN(gc1054_dual);
...
#define SENSOR_I2C_MASK    (0x02)
#define SENSOR_I2C_ADDR    0x42                   //I2C slave address
#define SENSOR_I2C_NEW_ADDR    (SENSOR_I2C_ADDR^SENSOR_I2C_MASK)  //I2C slave address
...
static I2C_ARRAY Sensor_init_table[] =
{
    ...
    {0xfb,SENSOR_I2C_NEW_ADDR},
};

I2C_ARRAY Current_Mirror_Flip_Tbl_dual[] = {
     {0x17, 0xC0},    // bit[1:0]
};
...
#endif
#undef SENSOR_NAME
#define SENSOR_NAME gc1054_dual
...
static int pCus_init(ms_cus_sensor *handle)
{
    ...
    //TStart = timeGetTimeU();
    if(handle->i2c_cfg.address !=  SENSOR_I2C_ADDR)
    {
        handle->i2c_cfg.address =  SENSOR_I2C_ADDR;
        if(SensorReg_Write(0xfb,SENSOR_I2C_NEW_ADDR)!= SUCCESS)
        {
            SENSOR_DMSG("[%s:%d]Sensor write 0xfb fail!!\n", __FUNCTION__, __LINE__);
        }
...
static int pCus_SetOrien(ms_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit) {
    ...
    switch(orit)
    {
        case CUS_ORIT_M0F0:
            ...
            Current_Mirror_Flip_Tbl_dual[0].reg = mirr_flip_table[0].reg;
            Current_Mirror_Flip_Tbl_dual[0].data = mirr_flip_table[0].data;
        ...
        case CUS_ORIT_M1F0:
            ...
            Current_Mirror_Flip_Tbl_dual[0].reg = mirr_flip_table[1].reg;
            Current_Mirror_Flip_Tbl_dual[0].data = mirr_flip_table[1].data;
    ...
    //  handle->bayer_id= CUS_BAYER_BG;
        ...
        case CUS_ORIT_M0F1:
            ...
            Current_Mirror_Flip_Tbl_dual[0].reg = mirr_flip_table[2].reg;
            Current_Mirror_Flip_Tbl_dual[0].data = mirr_flip_table[2].data;
     ...
     // handle->bayer_id= CUS_BAYER_GR;
        ...
        case CUS_ORIT_M1F1:
            ...
            Current_Mirror_Flip_Tbl_dual[0].reg = mirr_flip_table[3].reg;
            Current_Mirror_Flip_Tbl_dual[0].data = mirr_flip_table[3].data;
            handle->bayer_id = CUS_BAYER_BG;
     // handle->bayer_id= CUS_BAYER_GR;
            break;
        default :
...
static int pCus_AEStatusNotify(ms_cus_sensor *handle, CUS_CAMSENSOR_AE_STATUS_NOTIFY status){
    ...
    switch(status)
    {
        ...
        {
            SensorReg_Write(0xfe,0x00);//page 0
            SensorReg_Write(Current_Mirror_Flip_Tbl_dual[0].reg,Current_Mirror_Flip_Tbl_dual[0].data);
        }
        break;
        default :
        break;
...
static int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ...
   //ISensorIfAPI *sensor_if = handle->sensor_if_api;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    strcpy(handle->model_id,"gc1054_dual_MIPI");
...
//    handle->pCus_sensor_SetAEGain_cal   = pCus_SetAEGain_cal;
    ...
    handle->sensor_if_api->Reset(0,0);
...
SENSOR_DRV_ENTRY_IMPL_END(  gc1054_dual,
