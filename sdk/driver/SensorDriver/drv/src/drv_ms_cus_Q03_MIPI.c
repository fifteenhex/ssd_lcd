...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(Q03);
...
#define Preview_line_period 24242//30580                  //(36M/37.125M)*30fps=29.091fps(34.375msec), hts=34.375/1125=30556,
#define Preview_line_period_HDR 24242//30580                  //(36M/37.125M)*30fps=29.091fps(34.375msec), hts=34.375/1125=30556,
#define vts_30fps  1375//1266//1150//1090                              //for 29.091fps @ MCLK=36MHz
#define vts_30fps_HDR  2750//1266//1150//1090                              //for 29.091fps @ MCLK=36MHz
#define Preview_WIDTH       2304//1920                    //resolution Width when preview
#define Preview_HEIGHT      1296//1080                    //resolution Height when preview
...
static I2C_ARRAY mirror_reg[] =
{
    ...
    {0x28, 0x15}, // crop for bayer mirror
    {0x80, 0x00}, //
};

static I2C_ARRAY mirror_reg_HDR[] =
{
    {0x12, 0x08}, // mirror bit[5] HDR on/off[3]
...
const I2C_ARRAY expo_reg_HDR_SEF[] = {
    ...
    {0x05, 0xff}, //long expo[7:0]
};

const I2C_ARRAY vts_reg[] = {
    {0x23,0x05},
    {0x22,0x5f},    //frame H 1375
};
...
static int pCus_poweroff(ms_cus_sensor *handle, u32 idx)
{
    // power/reset low
   ISensorIfAPI *sensor_if = handle->sensor_if_api;
//    Q03_params *params = (Q03_params *)handle->private_data;
...
static int pCus_GetSensorID(ms_cus_sensor *handle, u32 *id)
{
    ...
    *id >>= 8;
    SENSOR_DMSG("[%s]Q03 Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);
    //SENSOR_DMSG("[%s]Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);

    return SUCCESS;
}

static int Q03_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
...
static int pCus_SetOrien(ms_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit) {
    ...
    switch(orit) {
        case CUS_ORIT_M0F0:
            ...
            params->tMirror_reg[1].data = 0x15;
        ...
        case CUS_ORIT_M1F0:
            ...
            params->tMirror_reg[1].data = 0x15;
            params->tMirror_reg[2].data = 0x40;
            params->ori_dirty = true;
        break;
        case CUS_ORIT_M0F1:
            params->tMirror_reg[0].data = 0x10;
            params->tMirror_reg[1].data = 0x16;
            params->tMirror_reg[2].data = 0x00;
            params->ori_dirty = true;
        break;
        case CUS_ORIT_M1F1:
...
static int pCus_SetOrien_HDR(ms_cus_sensor *handle, CUS_CAMSENSOR_ORIT orit) {
    ...
    switch(orit) {
        case CUS_ORIT_M0F0:
            ...
            params->tMirror_reg_HDR[2].data = 0x00;
            params->ori_dirty = true;
        break;
        case CUS_ORIT_M1F0:
            params->tMirror_reg_HDR[0].data = 0x28;
            params->tMirror_reg_HDR[1].data = 0x25;
            params->tMirror_reg_HDR[2].data = 0x40;
            params->ori_dirty = true;
        break;
        case CUS_ORIT_M0F1:
            params->tMirror_reg_HDR[0].data = 0x18;
            params->tMirror_reg_HDR[1].data = 0x27;
            params->tMirror_reg_HDR[2].data = 0x00;
            params->ori_dirty = true;
        break;
        case CUS_ORIT_M1F1:
...
static int Q03_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int Q03_GetShutterInfo_HDR_LEF(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    info->max = 1000000000/Preview_MIN_FPS; ///12;
...
static int Q03_GetShutterInfo_HDR_SEF(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    Q03_params *params = (Q03_params *)handle->private_data;
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
    ...
    params = (Q03_params *)handle->private_data;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    SENSOR_DMSG(handle->model_id,"Q03_MIPI");
    ...
    handle->pCus_sensor_SetPatternMode = Q03_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
...
//    handle->pCus_sensor_SetAEGain_cal   = pCus_SetAEGain_cal;
    ...
    handle->pCus_sensor_GetShutterInfo = Q03_GetShutterInfo;
...
{
    ms_cus_sensor *handle = drv_handle;
    Q03_params *params = NULL;

    cus_camsensor_init_handle(drv_handle);
    params = (Q03_params *)handle->private_data;

    sprintf(handle->model_id,"Q03_MIPI_HDR_SEF");
    ...
    handle->video_res_supported.res[0].nOutputWidth = 2304;
    handle->video_res_supported.res[0].nOutputHeight = 1296;
    sprintf(handle->video_res_supported.res[0].strResDesc, "2304x1296@30fps_HDR");
    ...
    //handle->pCus_sensor_SetAEGain       = pCus_SetAEGain;
    handle->pCus_sensor_SetAEGain       = pCus_SetAEGain;
    handle->pCus_sensor_GetShutterInfo = Q03_GetShutterInfo_HDR_SEF;
...
int cus_camsensor_init_handle_HDR_LEF(ms_cus_sensor* drv_handle) {
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->model_id,"Q03_MIPI_HDR_LEF");
    ...
    handle->video_res_supported.res[0].nOutputWidth= 2304;
    handle->video_res_supported.res[0].nOutputHeight= 1296;
    sprintf(handle->video_res_supported.res[0].strResDesc, "2304x1296@15fps_HDR");
    ...
    handle->pCus_sensor_GetShutterInfo = Q03_GetShutterInfo_HDR_LEF;
    handle->pCus_sensor_CustDefineFunction = pCus_sensor_CustDefineFunction;
...
SENSOR_DRV_ENTRY_IMPL_END_EX(  Q03,
