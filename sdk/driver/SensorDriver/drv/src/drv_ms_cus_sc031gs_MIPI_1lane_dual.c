...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(sc031gs_dual);
...
#define Preview_MAX_FPS     120  //25                     //fastest preview FPS
#define Preview_MIN_FPS     5                       //slowest preview FPS
#define Preview_CROP_START_X     0                      //CROP_START_X
#define Preview_CROP_START_Y     0                      //CROP_START_Y

#define SENSOR_I2C_ADDR    0x66                   //I2C slave address
#define SENSOR_I2C_NEW_ADDR    0x66  //I2C slave address
...
const static I2C_ARRAY Sensor_init_table_30W30fps[] =
{
    {0x0103,0x01},
    {0x0100,0x00},
    {0x36e9,0x80},
...
#endif
#if 0  //For customer: dual sensor on p3 qfn68
static	unsigned int gpio_pdn1 = 11; //For muxin
static void pDn_set(void)
{
    int error;

    error = camdriver_gpio_request(NULL, gpio_pdn1);
    if(error) SENSOR_DMSG("[%s] Request PDN1 failed\n", __FUNCTION__);

    /* begin select I2C slave addr */
    error = camdriver_gpio_direction_output(NULL, gpio_pdn1, 1);
    if(error) SENSOR_DMSG("[%s] Output PDN0 failed\n", __FUNCTION__);
...
//    sensor_if->Reset(idx, handle->reset_POLARITY);
    ...
    //pDn_set(); //For custmoer: dual sensor on p3 qfn68
...
static int pCus_poweroff(ms_cus_sensor *handle, u32 idx)
{
    // power/reset low
    ISensorIfAPI *sensor_if = handle->sensor_if_api;
    sc031gs_params *params = (sc031gs_params *)handle->private_data;
    SENSOR_DMSG("[%s] power low\n", __FUNCTION__);
//    sensor_if->PowerOff(idx, handle->pwdn_POLARITY);
//    sensor_if->Reset(idx, handle->reset_POLARITY);
...
static int sc031gs_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
  int i;
  SENSOR_DMSG("\n\n[%s], mode=%d \n", __FUNCTION__, mode);
...
static int pCus_init_linear_30W30fps(ms_cus_sensor *handle)
{
    sc031gs_params *params = (sc031gs_params *)handle->private_data;
    //SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    int i,cnt;
    ...
    /* Reset I2C slave address */
    handle->i2c_cfg.address = SENSOR_I2C_NEW_ADDR;
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_30W30fps);i++)
    {
        if(Sensor_init_table_30W30fps[i].reg==0xffff)
        {
            SENSOR_MSLEEP(Sensor_init_table_30W30fps[i].data);
        }
        else
        {
            cnt = 0;
            while(SensorReg_Write(Sensor_init_table_30W30fps[i].reg, Sensor_init_table_30W30fps[i].data) != SUCCESS)
            {
                cnt++;
...
static int pCus_SetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        case 0: //"640*480@120fps"
            handle->video_res_supported.ulcur_res = 0;
            handle->pCus_sensor_init = pCus_init_linear_30W30fps;
            vts_30fps= 683;//1500
            params->expo.vts = vts_30fps;
            params->expo.fps = 60;
            Preview_line_period  = 12201;
            break;
...
static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain) {
    ...
    else if (gain < 4096) // 4 * 1024
    {
        Dcg_gainx100 = 1000;      Coarse_gain = 2;     DIG_gain=1;       DIG_Fine_gainx1000=1000;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x07; DIG_gain_reg=0x0;  DIG_Fine_gain_reg=0x80;
    }
    else if (gain < 8192) // 8 * 1024
    {
        Dcg_gainx100 = 1000;      Coarse_gain = 4;     DIG_gain=1;       DIG_Fine_gainx1000=1000;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x0f; DIG_gain_reg=0x0;  DIG_Fine_gain_reg=0x80;

    }
    else if (gain <= 15872)// 15.500 * 1024; 16 * 1024 = 16384
    {
        Dcg_gainx100 = 1000;      Coarse_gain = 8;     DIG_gain=1;       DIG_Fine_gainx1000=1000;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x1f; DIG_gain_reg=0x0;  DIG_Fine_gain_reg=0x80;

    }
    else if (gain < 15872 * 2) // start dgain 31744
    {
        Dcg_gainx100 = 1000;      Coarse_gain = 8;     DIG_gain=1;       ANA_Fine_gainx64=127;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x1f; DIG_gain_reg=0x0;  ANA_Fine_gain_reg=0x1f;
    }
    else if (gain < 15872 * 4) // 63488
    {
        Dcg_gainx100 = 1000;      Coarse_gain = 8;     DIG_gain=2;       ANA_Fine_gainx64=127;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x1f; DIG_gain_reg=0x1;  ANA_Fine_gain_reg=0x1f;
    }
    else if (gain <= 123008)   //
    {
        Dcg_gainx100 = 1000;      Coarse_gain = 8;     DIG_gain=4;       ANA_Fine_gainx64=127;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x1f; DIG_gain_reg=0x3;  ANA_Fine_gain_reg=0x1f;
    }

    if(gain <= 15872)
    {
        ANA_Fine_gain_reg = abs(1000 * gain / (Dcg_gainx100 * Coarse_gain) / 64);
    }else{
        DIG_Fine_gain_reg = abs(8000 * gain /(Dcg_gainx100 * Coarse_gain * DIG_gain) / ANA_Fine_gainx64);
        if (DIG_Fine_gain_reg < 0x80)
    ...
    if (gain_reg_temp[2].data < 0x07) {         // gain < x2
        ...
        params->tNr_reg[1].data = 0x10;
    }
    else // if (gain_reg_temp[2].data < 0x0F) {     // gain >= x2
    {
        params->tNr_reg[0].data = 0x4f;
        params->tNr_reg[1].data = 0x0f;
    }
...
static int sc031gs_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    params = (sc031gs_params *)handle->private_data;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    strcpy(handle->model_id,"sc031gs_MIPI_dual");
    ...
    sprintf(handle->video_res_supported.res[0].strResDesc, "640*480@120fps");
    ...
    handle->pCus_sensor_SetPatternMode = sc031gs_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo = sc031gs_GetShutterInfo;
    params->expo.vts=vts_30fps;
    params->expo.fps = 90;
...
SENSOR_DRV_ENTRY_IMPL_END_EX( sc031gs_dual,
