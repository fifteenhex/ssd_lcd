...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(OV4689);

#if 1//(SENSOR0 == OV4689_MIPI)
...
#endif
#define OV4689_2432x1368_60P_HDR (1) //Verify FPGA
...
#define SENSOR_MAX_GAIN     15872//(15.5 * 1024)                // max sensor again, a-gain * conversion-gain*d-gain
...
const static I2C_ARRAY Sensor_init_table_4lane_1288x724_60fps[] =
{
    /*
...
static int OV4689_poweron( ms_cus_sensor *handle,  u32 idx)
{
   ...
   //     sensor_if->SetCSI_hdr_mode(idx, handle->interface_attr.attr_mipi.mipi_hdr_mode, 1);
    //}
    sensor_if->SetCSI_clk_data_skip(idx, 0xE, 0x6);
...
static int OV4689_poweroff( ms_cus_sensor *handle,  u32 idx)
{
    ISensorIfAPI *sensor_if = handle->sensor_if_api;
    ...
    *id = 0;
    ...
    for( n = 0; n < 4; ++n )            //retry , until I2C success
    {
        ...
        if( SensorRegArrayR((I2C_ARRAY*)id_from_sensor,table_length) == SUCCESS )            //read sensor ID from I2C
        {
            ...
            SENSOR_USLEEP(1000);//MsSleep(RTK_MS_TO_TICK(1));//usleep( 1*1000 );
    }
...
static int OV4689_init( ms_cus_sensor *handle )
{
    ...
    for( i = 0; i < ArraySize; i++ )
    {
        if( (pSensor_init_table+i)->reg == 0xffff ){
            SENSOR_USLEEP(1000);//MsSleep(RTK_MS_TO_TICK(1));//usleep( 1000*Sensor_init_table_R1C[i].data );
        }
        else{
...
static int OV4689_GetVideoRes(ms_cus_sensor *handle, u32 res_idx, cus_camsensor_res **res)
{
...
static int OV4689_GetCurVideoRes(ms_cus_sensor *handle, u32 *cur_idx, cus_camsensor_res **res)
{
    u32 num_res = handle->video_res_supported.num_res;
...
static int OV4689_SetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
...
static int OV4689_SetFPS( ms_cus_sensor *handle, u32 fps )
{
    ...
    if( fps >= 2 && fps <= 60 )
    {
        ...
        return SensorRegArrayW( ( I2C_ARRAY* )params->tVts_reg, ARRAY_SIZE(vts_reg) );
    }
...
static int DRIVER_SetAEGain( ms_cus_sensor *handle, u32 gain )
{
    ...
    else if((gain>=4096 )&&(gain<8192))            //X4~X8
    {
        ...
        bEnableBlackBadPixelComp = false;
        gain4_8=gain-4096;
        params->tGain_reg[0].data =( (gain4_8>>8) & 0x000f)|0x30;
    }
    else if((gain>=8192 )&&(gain<16384))            //X8~X16
    {
        ...
        bEnableBlackBadPixelComp = false;
        gain8_16=gain-8192;
        params->tGain_reg[0].data =( (gain8_16>>9) & 0x000f)|0x70;
    }
    else if((gain>=16384 )&&(gain<32768))            //X16~X32
    {
        ...
        params->tGain_reg[0].data =( (gain16_32>>10) & 0x000f)|0xf0;
    }
...
int ov4689_MIPI_cus_camsensor_init_handle( ms_cus_sensor* drv_handle )
{
    ...
    handle->video_res_supported.res[RES_IDX_2304x1296_60P_HDR].crop_start_x = 0;
    handle->video_res_supported.res[RES_IDX_2304x1296_60P_HDR].crop_start_y = 0;
    ...
    sprintf(handle->video_res_supported.res[RES_IDX_2304x1296_60P_HDR].strResDesc, "2304x1296@30fps");
    ...
    sprintf(handle->video_res_supported.res[RES_IDX_1920x1080_30P].strResDesc, "1920x1080@30fps");
    ...
    sprintf(handle->video_res_supported.res[RES_IDX_1280x720_60P].strResDesc, "1280x720@30fps");
...
SENSOR_DRV_ENTRY_IMPL_END_EX(  OV4698,
