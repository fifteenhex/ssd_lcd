...
#define SENSOR_SHUTTER_DELAY_FRAME_COUNT_DOL    (2)
 ...
 //CUS_CMU_SCL_CLK_37P125M        //CFG //CUS_CMU_CLK_12M, CUS_CMU_CLK_16M, CUS_CMU_CLK_24M, CUS_CMU_CLK_27M
...
//For MCLK = 37.125Mhz
#define Preview_line_period_HDR_DOL_4LANE 27322 / 2        //hts=33.333/1220=27322
#endif
#if HDR_DOL_12bit
//For MCLK = 36Mhz
...
u32 Preview_MCLK_SPEED = CUS_CMU_CLK_37P125MHZ;
...
const static I2C_ARRAY Sensor_init_table_2lane_Mclk36Mhz[] =
{
...
static int pCus_init_mipi2lane_linear_Mclk36Mhz(ms_cus_sensor *handle)
{
    int i,cnt=0;
    SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_init_table_2lane_Mclk36Mhz);i++)
    {
		if(Sensor_init_table_2lane_Mclk36Mhz[i].reg==0xffff)
		{
			SENSOR_MSLEEP_(Sensor_init_table_2lane_Mclk36Mhz[i].data);
		}
		else
		{
			cnt = 0;
			while(SensorReg_Write(Sensor_init_table_2lane_Mclk36Mhz[i].reg,Sensor_init_table_2lane_Mclk36Mhz[i].data) != SUCCESS)
			{
				cnt++;
				if(cnt>=10)
				{
...
static int pCus_SetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        ...
        case 1:
            ...
            handle->pCus_sensor_init = pCus_init_mipi2lane_linear_Mclk36Mhz;
            params->expo.vts=vts_30fps;
            handle->mclk = CUS_CMU_CLK_36MHZ;
            break;
        default:
            break;
    }
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle)
{
    ...
    handle->video_res_supported.ulcur_res = 1; //default resolution index is 1.
    ...
    handle->video_res_supported.res[1].nOutputWidth = 0x79C;
    handle->video_res_supported.res[1].nOutputHeight = 0x449;
    sprintf(handle->video_res_supported.res[1].strResDesc, "1920x1080@30fps_MCLK36Mhz");
    // i2c
