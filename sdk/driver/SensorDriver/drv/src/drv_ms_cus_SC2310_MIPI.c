...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(SC2310_MIPI);
...
#define SENSOR_MAXGAIN      (4284*315 / 1000)
...
typedef struct {
    ...
    I2C_ARRAY tGain_reg_HDR_DOL_SEF[4];
...
const I2C_ARRAY Sensor_init_table_HDR_DOL[] =
{
    {0x0103,0x01},
    {0x0100,0x00},
...
const static I2C_ARRAY gain_reg_HDR_DOL_SEF[] = {
    {0x3e10,0x00},
    {0x3e11, 0x00},
    {0x3e12, 0x00|0x03},
...
const I2C_ARRAY expo_reg[] = {  // max expo line vts*2-6!
    {0x3e00, 0x00},//expo [20:17]
    {0x3e01, 0x8c}, // expo[16:8]
    {0x3e02, 0x40}, // expo[7:0], [3:0] fraction of line
};

const I2C_ARRAY expo_reg_HDR_DOL_SEF[] = {
...
const I2C_ARRAY max_short_exp_reg[] = {
    {0x3e23, 0x00},
    {0x3e24, 0x87}
};
...
static int pCus_GetSensorID(ms_cus_sensor *handle, u32 *id)
{
  ...
  SENSOR_DMSG("\n\n[%s]", __FUNCTION__);
    ...
    *id >>= 8;
    SENSOR_DMSG("[%s]sc2310 Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);
    //SENSOR_DMSG("[%s]Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);

    return SUCCESS;
}

static int sc2310_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
...
static int pCus_init_mipi2lane_linear_2M30fps(ms_cus_sensor *handle)
{
    ...
    {
        if(0x0100 == Sensor_init_table_2M30fps[i].reg && 0x01 == Sensor_init_table_2M30fps[i].data)
        {
...
static int pCus_init_mipi2lane_HDR_DOL(ms_cus_sensor *handle)
{
    ...
    {
        if(0x0100 == Sensor_init_table_HDR_DOL[i].reg && 0x01 == Sensor_init_table_HDR_DOL[i].data)
        {
            sensor_if->SetCSI_Clk(handle->i2c_bus->nSensorID, CUS_CSI_CLK_216M);
            sensor_if->SetCSI_Lane(handle->i2c_bus->nSensorID, handle->interface_attr.attr_mipi.mipi_lane_num, 1);
            sensor_if->SetCSI_LongPacketType(handle->i2c_bus->nSensorID, 0, 0x1C00, 0);
    ...
    switch(orit)
    {
        ...
        case CUS_ORIT_M1F0:
            for(i=seg_length,j=0;i<seg_length*2;i++,j++){
                SensorReg_Write(mirr_flip_table[i].reg,mirr_flip_table[i].data);
                params->tMirror_table[0].data = 0x06;
        ...
        case CUS_ORIT_M0F1:
            for(i=seg_length*2,j=0;i<seg_length*3;i++,j++){
                SensorReg_Write(mirr_flip_table[i].reg,mirr_flip_table[i].data);
                params->tMirror_table[0].data = 0x60;
        ...
        case CUS_ORIT_M1F1:
            for(i=seg_length*3,j=0;i<seg_length*4;i++,j++){
                SensorReg_Write(mirr_flip_table[i].reg,mirr_flip_table[i].data);
                params->tMirror_table[0].data = 0x66;
...
static int pCus_AEStatusNotifyHDR_DOL_LEF(ms_cus_sensor *handle, CUS_CAMSENSOR_AE_STATUS_NOTIFY status)
{
    ...
    switch(status)
    {
        ...
        {
           SensorRegArrayW((I2C_ARRAY*)params->tExpo_reg, sizeof(expo_reg)/sizeof(I2C_ARRAY));
           SensorRegArrayW((I2C_ARRAY*)params->tExpo_reg_HDR_DOL_SEF, sizeof(expo_reg_HDR_DOL_SEF)/sizeof(I2C_ARRAY));
           SensorRegArrayW((I2C_ARRAY*)params->tGain_reg, sizeof(gain_reg)/sizeof(I2C_ARRAY));
           SensorRegArrayW((I2C_ARRAY*)params->tGain_reg_HDR_DOL_SEF, sizeof(gain_reg_HDR_DOL_SEF)/sizeof(I2C_ARRAY));
           SensorRegArrayW((I2C_ARRAY*)params->tVts_reg, sizeof(vts_reg)/sizeof(I2C_ARRAY));
           params->reg_dirty = false;
        }
        break;
...
static int pCus_SetAEUSecsHDR_DOL_LEF(ms_cus_sensor *handle, u32 us)
{
    ...
    if (half_lines >  2 * (params->expo.vts-params->expo.max_short_exp-5)) {
        half_lines = 2 * (params->expo.vts-params->expo.max_short_exp-5);
    }
    else
     vts=params->expo.vts;
...
static int pCus_GetAEUSecsHDR_DOL_SEF(ms_cus_sensor *handle, u32 *us) {
    ...
    u32 lines = 0;
    sc2310_params *params = (sc2310_params *)handle->private_data;
    lines |= (u32)(params->tExpo_reg_HDR_DOL_SEF[0].data&0x0f)<<8;
    lines |= (u32)(params->tExpo_reg_HDR_DOL_SEF[1].data&0xff)<<0;
    lines >>= 4;
...
static int pCus_SetAEUSecsHDR_DOL_SEF(ms_cus_sensor *handle, u32 us)
{
    ...
    memcpy(expo_reg_temp, params->tExpo_reg_HDR_DOL_SEF, sizeof(expo_reg_HDR_DOL_SEF));

    dou_lines = (1000*us)/(Preview_line_period_HDR_DOL*2); // Preview_line_period in ns
    half_lines = 4*dou_lines;
    if(half_lines<8) half_lines=8;
    if (half_lines >  2 * (params->expo.max_short_exp-4)) {
        half_lines = 2 * (params->expo.max_short_exp-4);
    }
    else
     vts=params->expo.vts;
//  SENSOR_DMSG("[%s] us %ld, half_lines %ld, vts %ld\n", __FUNCTION__, us, half_lines, params->expo.vts);
    ...
    params->tExpo_reg_HDR_DOL_SEF[0].data =  (half_lines>>8) & 0xff;
    params->tExpo_reg_HDR_DOL_SEF[1].data = (half_lines>>0) & 0xf0;
 //   pr_info("[%s]  expo_reg_HDR_DOL_SEF : %x , %x\n\n", __FUNCTION__,expo_reg_HDR_DOL_SEF[0].data,expo_reg_HDR_DOL_SEF[1].data);
    ...
    {
      if (params->tExpo_reg_HDR_DOL_SEF[i].data != expo_reg_temp[i].data)
      {
        params->reg_dirty = true;
        break;
      }
...
static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain) {
    ...
    {
        Dcg_gainx100 = 272;      Coarse_gain = 8;     DIG_gain=1;       ANA_Fine_gainx64=127;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x3f; DIG_gain_reg=0x0;  ANA_Fine_gain_reg=0x7f;

    }
    else if (gain < 176865)
    {
        Dcg_gainx100 = 272;      Coarse_gain = 8;     DIG_gain=2;       ANA_Fine_gainx64=127;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x3f; DIG_gain_reg=0x1;  ANA_Fine_gain_reg=0x7f;
    }
    else if (gain < 353730)
    {
        Dcg_gainx100 = 272;      Coarse_gain = 8;     DIG_gain=4;       ANA_Fine_gainx64=127;
        Dcg_gain_reg = 1;  Coarse_gain_reg = 0x3f; DIG_gain_reg=0x3;  ANA_Fine_gain_reg=0x7f;
    }
   else if (gain < 707461)
    {
       Dcg_gainx100 = 272;      Coarse_gain = 8;      DIG_gain=8;        ANA_Fine_gainx64=127;
       Dcg_gain_reg = 1;  Coarse_gain_reg = 0x3f; DIG_gain_reg=0x7;  ANA_Fine_gain_reg=0x7f;
    }
    else if (gain < SENSOR_MAXGAIN * 1024)
    {
        Dcg_gainx100 = 272;      Coarse_gain = 8;     DIG_gain=16;       ANA_Fine_gainx64=127;
...
static int pCus_SetAEGainHDR_DOL_SEF(ms_cus_sensor *handle, u32 gain) {
    ...
    memcpy(gain_reg_temp, params->tGain_reg_HDR_DOL_SEF, sizeof(gain_reg_HDR_DOL_SEF));
    ...
    params->tGain_reg_HDR_DOL_SEF[3].data = ANA_Fine_gain_reg;
    params->tGain_reg_HDR_DOL_SEF[2].data = Coarse_gain_reg;
    params->tGain_reg_HDR_DOL_SEF[1].data = DIG_Fine_gain_reg;
    params->tGain_reg_HDR_DOL_SEF[0].data = DIG_gain_reg & 0xF;
    ...
    {
      if (params->tGain_reg_HDR_DOL_SEF[i].data != gain_reg_temp[i].data)
      {
        params->reg_dirty = true;
        break;
      }
...
static int sc2310_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle_linear(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    SENSOR_DMSG(handle->model_id,"sc2310_MIPI");
    ...
    handle->pCus_sensor_SetPatternMode = sc2310_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo = sc2310_GetShutterInfo;
    params->expo.vts=vts_30fps;
    params->expo.fps = 30;
...
{
    ms_cus_sensor *handle = drv_handle;
    sc2310_params *params = NULL;

    cus_camsensor_init_handle_linear(drv_handle);
    params = (sc2310_params *)handle->private_data;

    sprintf(handle->model_id,"sc2310_MIPI_HDR_SEF");
...
int cus_camsensor_init_handle_hdr_dol_lef(ms_cus_sensor* drv_handle)
{
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    sprintf(handle->model_id,"SC2310_MIPI_HDR_LEF");
...
SENSOR_DRV_ENTRY_IMPL_END_EX(  SC2310_MIPI,
