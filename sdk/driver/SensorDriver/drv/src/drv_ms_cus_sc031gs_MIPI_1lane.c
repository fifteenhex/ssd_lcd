...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(sc031gs);
...
static	unsigned int gpio_pdn0 = 10; //For muxin
static void pDn_set(void)
{
    int error;

    error = camdriver_gpio_request(NULL, gpio_pdn0);
        if(error) SENSOR_DMSG("[%s] Request PDN0 failed\n", __FUNCTION__);

    /* begin select I2C slave addr */
    error = camdriver_gpio_direction_output(NULL, gpio_pdn0, 1);
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    strcpy(handle->model_id,"sc031gs_MIPI");
...
SENSOR_DRV_ENTRY_IMPL_END_EX( sc031gs,
