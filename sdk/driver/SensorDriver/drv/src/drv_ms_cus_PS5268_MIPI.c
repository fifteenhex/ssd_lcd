...
#include <PS5268_MIPI_init_table.h>

#ifdef __cplusplus
}
#endif

SENSOR_DRV_ENTRY_IMPL_BEGIN(PS5268);
...
// MCLK Info                      //
////////////////////////////////////
#define Preview_MCLK_SPEED          CUS_CMU_CLK_27MHZ    //CUS_CMU_CLK_24MHZ //CUS_CMU_CLK_37P125MHZ
...
const static I2C_ARRAY Sensor_id_table[] =
{
    ...
    {0x0101, 0x68}, // {low byte address of ID, ID_L},
};
...
static int pCus_GetSensorID(ms_cus_sensor *handle, u32 *id)
{
    ...
    *id >>= 8;
    SENSOR_DMSG("[%s]ps5268 Read sensor id, get 0x%x Success\n", __FUNCTION__, (int)*id);
...
static int ps5268_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
    SENSOR_DMSG("\n\n[%s], mode=%d \n", __FUNCTION__, mode);
    return SUCCESS;
}
...
{
    ps5268_params *params = (ps5268_params *)handle->private_data;
...
static int ps5268_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
    info->max = 1000000000 / ps5268_mipi_linear[handle->video_res_supported.ulcur_res].senout.min_fps;
...
static int cus_camsensor_init_handle_linear(ms_cus_sensor* drv_handle)
{
    ...
    params = (ps5268_params *)handle->private_data;
    ...
    //    sensor model ID             //
    ////////////////////////////////////
    strcpy(handle->model_id,"ps5268_MIPI");
    ...
    for (res = 0; res < LINEAR_RES_END; res++) {
        handle->video_res_supported.res[res].width         = ps5268_mipi_linear[res].senif.preview_w;
        handle->video_res_supported.res[res].height        = ps5268_mipi_linear[res].senif.preview_h;
        handle->video_res_supported.res[res].max_fps       = ps5268_mipi_linear[res].senout.max_fps;
        handle->video_res_supported.res[res].min_fps       = ps5268_mipi_linear[res].senout.min_fps;
        handle->video_res_supported.res[res].crop_start_x  = ps5268_mipi_linear[res].senif.crop_start_X;
        handle->video_res_supported.res[res].crop_start_y  = ps5268_mipi_linear[res].senif.crop_start_y;
        handle->video_res_supported.res[res].nOutputWidth  = ps5268_mipi_linear[res].senout.width;
        handle->video_res_supported.res[res].nOutputHeight = ps5268_mipi_linear[res].senout.height;
        sprintf(handle->video_res_supported.res[res].strResDesc, ps5268_mipi_linear[res].senstr.strResDesc);
    }
    ...
    handle->pCus_sensor_SetPatternMode = ps5268_SetPatternMode;
    ...
    handle->pCus_sensor_GetShutterInfo  = ps5268_GetShutterInfo;
...
SENSOR_DRV_ENTRY_IMPL_END_EX( PS5268,
