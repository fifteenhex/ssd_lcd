...
static int pCus_init_2m_60fps_mipi4lane_linear(ms_cus_sensor *handle)
{
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_2m_60fps_init_table_4lane_linear);i++)
    {
        ...
        else
        {
            ...
            //printk("\n reg 0x%x, 0x%x",Sensor_2m_60fps_init_table_4lane_linear[i].reg, Sensor_2m_60fps_init_table_4lane_linear[i].data);
#if 0
            SensorReg_Read(Sensor_2m_60fps_init_table_4lane_linear[i].reg, &sen_data );
            if(Sensor_2m_60fps_init_table_4lane_linear[i].data != sen_data)
                printk("R/W Differ Reg: 0x%x\n",Sensor_2m_60fps_init_table_4lane_linear[i].reg);
                //printk("IMX415 reg: 0x%x, data: 0x%x, read: 0x%x.\n",Sensor_2m_60fps_init_table_4lane_linear[i].reg, Sensor_2m_60fps_init_table_4lane_linear[i].data, sen_data);
#endif
        }
    }
...
static int pCus_SetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
    ...
    switch (res_idx) {
        ...
        //    handle->pCus_sensor_init = pCus_init_5m_25fps_mipi4lane_linear;
        //    vts_30fps = 2250;
        //    params->expo.vts = vts_30fps;
        //    params->expo.fps = 25;
        //    Preview_line_period = 14814;
