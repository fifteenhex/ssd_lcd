...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(C2390);

//c11 extern int usleep(u32 usec);
//int usleep(u32 usec);

#define SENSOR_CHANNEL_NUM (0)
#define SENSOR_CHANNEL_MODE CUS_SENSOR_CHANNEL_MODE_REALTIME_NORMAL //CUS_SENSOR_CHANNEL_MODE_RAW_STORE_LI_HDR //CUS_SENSOR_CHANNEL_MODE_REALTIME_NORMAL
...
#define SENSOR_I2C_SPEED    200000//I2C_SLOWSPEED_MODE_200KHZ   //200KHz
    ...
    *id = 0;
    ...
    for( n = 0; n < 4; ++n )            //retry , until I2C success
    {
        ...
        if( SensorRegArrayR((I2C_ARRAY*)id_from_sensor,table_length) == SUCCESS )            //read sensor ID from I2C
        {
            ...
            SENSOR_MSLEEP(1);//            MsSleep(RTK_MS_TO_TICK(1));//usleep( 1*1000 );
...
#if (SENSOR_MIPI_HDR_MODE == 2) //Merged from Mv2.
static int pCus_AEStatusNotifyHDR_HCG_LCG( ms_cus_sensor *handle, CUS_CAMSENSOR_AE_STATUS_NOTIFY status )
{
    static u8 ulFrameCnt = 0; //Merged from Mv2. //TBD
    u8 m_AeTime_ubPeriod = 3; //Merged from Mv2. //TBD

    switch( status )
    {
        case CUS_FRAME_ACTIVE:
            if ((ulFrameCnt % m_AeTime_ubPeriod) == 0) {
                SensorReg_Write(0x3467, 0x00); //Define Group 0 length to 7 registers
                SensorReg_Write(0x3464, 0x00); //Select Group 0 to be defined
                SensorReg_Write(0xB0B6, (u8)((new_shutter_l >> 8)&0xff)); //HCG_LCG
                SensorReg_Write(0xB0B7, (u8)((new_shutter_l << 0)&0xff));
                SensorReg_Write(0xB0B8, (u8)((new_shutter_s >> 8)&0xff)); //VS
                SensorReg_Write(0xB0B9, (u8)((new_shutter_s << 0)&0xff));
                SensorReg_Write(0xB15A, (u8)(HCG_DGain >> 8));
                SensorReg_Write(0xB15B, (u8)HCG_DGain);
                SensorReg_Write(0xB15C, (u8)(LCG_DGain >> 8));
                SensorReg_Write(0xB15D, (u8)LCG_DGain);
                SensorReg_Write(0xB15E, (u8)(VS_DGain >> 8));
                SensorReg_Write(0xB15F, (u8)VS_DGain);
                SensorReg_Write(0xB0BB, (u8)(((VS_AGain << 4)&0x30) + ((LCG_AGain << 2)&0x0C) + ((HCG_AGain << 0)&0x03)));
                SensorReg_Write(0x3464, 0x40); //Select launch post SOF, Group 0 for single start
                SensorReg_Write(0x3467, 0x01); //Do single start
            }
            else if ((ulFrameCnt % m_AeTime_ubPeriod) == 2) {
                SensorReg_Write(0x3467, 0x00); //Define Group 0 length to 7 registers
                SensorReg_Write(0x3464, 0x04); //Select Group 1 to be defined
                SensorReg_Write(0xB190, HCG_switch);
                SensorReg_Write(0x3464, 0x14); //Select launch post SOF, Group 0 for single start
                SensorReg_Write(0x3467, 0x01); //Do single start
            }
            break;
    ...
    ulFrameCnt = (ulFrameCnt + 1) % m_AeTime_ubPeriod;
...
static int pCus_SetAEUSecsHDR_HCG_LCG( ms_cus_sensor *handle, u32 us )
{
    u32 new_vsync = vts_30fps;

    new_shutter_l = (1000 * us) / Preview_line_period;
    new_shutter_l = C2390_GET_MIN(C2390_GET_MAX(new_shutter_l, 2), new_vsync - 2);
...
static int pCus_SetAEGainHDR_HCG_LCG( ms_cus_sensor *handle, u32 gain )
{
    u32 ae_gain_l    = SENSOR_GAIN_BASE;
    u32  ae_gain_l_tmp = SENSOR_GAIN_BASE;
    u32 s_gain_HCG, s_gain_LCG;
    ...
    s_gain_HCG = C2390_GET_MAX( ae_gain_l_tmp , 2 * SENSOR_GAIN_BASE );
    ...
    {
        HCG_AGain = 3;
        HCG_DGain = (s_gain_HCG >> HCG_AGain)*2;
    ...
    //LCG for HDR
    pCus_SetAEGainHDR_LCG_VS_Cal(ae_gain_l, &LCG_AGain, &LCG_DGain);
...
#endif //#if (SENSOR_MIPI_HDR_MODE == 2) //Merged from Mv2.
...
int c2390_MIPI_cus_camsensor_init_handle( ms_cus_sensor* drv_handle )
{
    ...
    params = (c2390_params *)handle->private_data;
    ...
    handle->video_res_supported.res[0].height = 1080*2;
    handle->video_res_supported.res[0].max_fps = Preview_MAX_FPS;
    handle->video_res_supported.res[0].min_fps = Preview_MIN_FPS;
    handle->video_res_supported.res[0].crop_start_x= Preview_CROP_START_X;
    handle->video_res_supported.res[0].crop_start_y= Preview_CROP_START_Y;
    handle->video_res_supported.res[0].nOutputWidth= 1920;//1928;//Preview_WIDTH;
    handle->video_res_supported.res[0].nOutputHeight= 1080*2; //1088;//Preview_HEIGHT;
    sprintf(handle->video_res_supported.res[0].strResDesc, "1920x2160@30fps");
...
int c2390_MIPI_cus_camsensor_init_handle_LI( ms_cus_sensor* drv_handle )
{
    ms_cus_sensor *handle = drv_handle;
    c2390_params *params;
    if( !handle )
    ...
    handle->interface_attr.attr_mipi.mipi_yuv_order = 0; //don't care in RGB pattern.
    handle->interface_attr.attr_mipi.mipi_hsync_mode = SENSOR_MIPI_HSYNC_MODE;
    handle->interface_attr.attr_mipi.mipi_hdr_mode = CUS_HDR_MODE_LI;
    handle->interface_attr.attr_mipi.mipi_hdr_virtual_channel_num = 0; //Short frame
    ...
    handle->video_res_supported.res[0].crop_start_y= Preview_CROP_START_Y;
    handle->video_res_supported.res[0].nOutputWidth= 1920;//1928;//Preview_WIDTH;
    handle->video_res_supported.res[0].nOutputHeight= 1080; //1088;//Preview_HEIGHT;
    sprintf(handle->video_res_supported.res[0].strResDesc, "1920x1080@30fpsLI");
...
int c2390_MIPI_cus_camsensor_init_handle_dummy( ms_cus_sensor* drv_handle )
{
    ms_cus_sensor *handle = drv_handle;
    c2390_params *params;
    if( !handle )
    ...
    handle->pCus_sensor_AEStatusNotify = pCus_AEStatusNotifyHDR_HCG_LCG;
    handle->pCus_sensor_GetAEUSecs = pCus_GetAEUSecs;
    handle->pCus_sensor_SetAEUSecs = pCus_SetAEUSecsHDR_HCG_LCG;
    handle->pCus_sensor_GetAEGain = pCus_GetAEGain;
    handle->pCus_sensor_SetAEGain_cal = pCus_SetAEGain_cal;
    handle->pCus_sensor_SetAEGain = pCus_SetAEGainHDR_HCG_LCG;
...
#endif //#if (SENSOR_MIPI_HDR_MODE == 2)
...
struct SensorMapTable_t gsensor_map_c2390[] = {
    ...
    { //Channel 1
        ...
        c2390_MIPI_cus_camsensor_init_handle_dummy,
        c2390_MIPI_cus_camsensor_release_handle
    },
#endif
};
...
SENSOR_DRV_ENTRY_IMPL_END_EX(  C2390,
                            c2390_MIPI_cus_camsensor_init_handle,
                            c2390_MIPI_cus_camsensor_init_handle_LI,
                            c2390_MIPI_cus_camsensor_init_handle_LI,
                            c2390_params
                         );
