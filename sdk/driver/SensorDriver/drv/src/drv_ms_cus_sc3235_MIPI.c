...
#define SENSOR_MAXGAIN      63       //  (15.5*31)   max sensor gain, a-gain*conversion-gain*d-gain
...
static int sc3235_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
  int i;
  SENSOR_DMSG("\n\n[%s], mode=%d \n", __FUNCTION__, mode);
...
static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain) {
    ...
    if (gain <=  16128)
    {
        //Fine_againx32 =gain*100000 / ((Dgain * Coarse_gain * Fine_dgainx128*1024)/ 100000);
        Fine_againx32 = abs(4 * gain / (Dgain * Coarse_gain * Fine_dgainx128));
        Fine_again_reg = Fine_againx32;
        Fine_dgain_reg = abs(4 * gain / (Dgain * Coarse_gain * Fine_againx32));
    }
    else
    {
        //Fine_dgainx128 =gain*100000 / (Dgain * Coarse_gain * Fine_againx32)*100000/ 1024;
        Fine_dgain_reg = abs(4 * gain / (Dgain * Coarse_gain * Fine_againx32));
    }
...
static int sc3235_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    strcpy(handle->model_id,"sc3235_MIPI");
    ...
    handle->pCus_sensor_SetPatternMode = sc3235_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo = sc3235_GetShutterInfo;
    params->expo.vts=vts_30fps;
    params->expo.fps = 30;
