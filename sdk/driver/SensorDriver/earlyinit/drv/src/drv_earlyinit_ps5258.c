...
#include "PS5258_MIPI_init_table.h"         /* Sensor initial table */

/* Sensor EarlyInit implementation */
SENSOR_EARLYINIY_ENTRY_IMPL_BEGIN(PS5258)
...
static EarlyInitSensorInfo_t _gPS5258Info =
{
...
*/
static unsigned int PS5258EarlyInitShutterAndFps( unsigned int nFpsX1000, unsigned int nShutterUs, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
#if 0
    ...
    expo_reg[0].u16Data = (lines>>8) & 0xff;
    expo_reg[1].u16Data = (lines>>0) & 0xff;
    ...
    _gPS5258Info.u32ShutterUs = nShutterUs;
    _gPS5258Info.u32FpsX1000 = nFpsX1000;
...
@retval Return the number of I2C data in pRegs
*/
static unsigned int PS5258EarlyInitGain( unsigned int u32GainX1024, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
#if 0
    ...
    _gPS5258Info.u32GainX1024 = u32GainX1024;
    return nNumRegs;
#else
    return 0;
#endif
...
static unsigned int PS5258EarlyInitGetSensorInfo(EarlyInitSensorInfo_t* pSnrInfo)
{
    if(pSnrInfo)
    {
        pSnrInfo->eBayerID      = E_EARLYINIT_SNR_BAYER_BG;
        pSnrInfo->ePixelDepth   = EARLYINIT_DATAPRECISION_12;
        pSnrInfo->u32FpsX1000   = _gPS5258Info.u32FpsX1000;
        pSnrInfo->u32Width      = _gPS5258Info.u32Width;
        pSnrInfo->u32Height     = _gPS5258Info.u32Height;
        pSnrInfo->u32GainX1024  = _gPS5258Info.u32GainX1024;
        pSnrInfo->u32ShutterUs  = _gPS5258Info.u32ShutterUs;
        pSnrInfo->u32ShutterShortUs = _gPS5258Info.u32ShutterShortUs;
        pSnrInfo->u32GainShortX1024 = _gPS5258Info.u32GainShortX1024;
        pSnrInfo->u8ResId       = _gPS5258Info.u8ResId;
        pSnrInfo->u8HdrMode     = _gPS5258Info.u8HdrMode;
...
SENSOR_EARLYINIY_ENTRY_IMPL_END( PS5258,
                                 Sensor_init_table,
                                 PS5258EarlyInitShutterAndFps,
                                 PS5258EarlyInitGain,
                                 PS5258EarlyInitGetSensorInfo
                                );
