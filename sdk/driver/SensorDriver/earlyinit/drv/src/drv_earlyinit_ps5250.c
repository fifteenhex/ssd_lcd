...
#include "PS5250_MIPI_init_table.h"         /* Sensor initial table */

/* Sensor EarlyInit implementation */
SENSOR_EARLYINIY_ENTRY_IMPL_BEGIN(PS5250)
...
static EarlyInitSensorInfo_t _gPS5250Info =
{
...
*/
static unsigned int PS5250EarlyInitShutterAndFps( unsigned int nFpsX1000, unsigned int nShutterUs, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
...
@retval Return the number of I2C data in pRegs
*/
static unsigned int PS5250EarlyInitGain( unsigned int u32GainX1024, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
...
static unsigned int PS5250EarlyInitGetSensorInfo(EarlyInitSensorInfo_t* pSnrInfo)
{
    if(pSnrInfo)
    {
        pSnrInfo->eBayerID      = E_EARLYINIT_SNR_BAYER_BG;
        pSnrInfo->ePixelDepth   = EARLYINIT_DATAPRECISION_12;
        pSnrInfo->u32FpsX1000   = _gPS5250Info.u32FpsX1000;
        pSnrInfo->u32Width      = _gPS5250Info.u32Width;
        pSnrInfo->u32Height     = _gPS5250Info.u32Height;
        pSnrInfo->u32GainX1024  = _gPS5250Info.u32GainX1024;
        pSnrInfo->u32ShutterUs  = _gPS5250Info.u32ShutterUs;
        pSnrInfo->u32ShutterShortUs = _gPS5250Info.u32ShutterShortUs;
        pSnrInfo->u32GainShortX1024 = _gPS5250Info.u32GainShortX1024;
        pSnrInfo->u8ResId       = _gPS5250Info.u8ResId;
        pSnrInfo->u8HdrMode     = _gPS5250Info.u8HdrMode;
...
SENSOR_EARLYINIY_ENTRY_IMPL_END( PS5250,
                                 Sensor_init_table,
                                 PS5250EarlyInitShutterAndFps,
                                 PS5250EarlyInitGain,
                                 PS5250EarlyInitGetSensorInfo
                                );
