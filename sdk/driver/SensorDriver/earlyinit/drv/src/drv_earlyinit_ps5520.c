...
#include "PS5520_MIPI_init_table.h"         /* Sensor initial table */

/* Sensor EarlyInit implementation */
SENSOR_EARLYINIY_ENTRY_IMPL_BEGIN(PS5520)
...
static EarlyInitSensorInfo_t _gPS5520Info =
{
...
*/
static unsigned int PS5520EarlyInitShutterAndFps( unsigned int nFpsX1000, unsigned int nShutterUs, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
    ...
    _gPS5520Info.u32ShutterUs = nShutterUs;
    _gPS5520Info.u32FpsX1000 = nFpsX1000;
...
@retval Return the number of I2C data in pRegs
*/
static unsigned int PS5520EarlyInitGain( unsigned int u32GainX1024, I2cCfg_t *pRegs, unsigned int nMaxNumRegs)
{
    ...
    _gPS5520Info.u32GainX1024 = u32GainX1024;
    return nNumRegs;
}

static unsigned int PS5520EarlyInitGetSensorInfo(EarlyInitSensorInfo_t* pSnrInfo)
{
    if(pSnrInfo)
    {
        pSnrInfo->eBayerID      = E_EARLYINIT_SNR_BAYER_BG;
        pSnrInfo->ePixelDepth   = EARLYINIT_DATAPRECISION_12;
        pSnrInfo->u32FpsX1000   = _gPS5520Info.u32FpsX1000;
        pSnrInfo->u32Width      = _gPS5520Info.u32Width;
        pSnrInfo->u32Height     = _gPS5520Info.u32Height;
        pSnrInfo->u32GainX1024  = _gPS5520Info.u32GainX1024;
        pSnrInfo->u32ShutterUs  = _gPS5520Info.u32ShutterUs;
        pSnrInfo->u32ShutterShortUs = _gPS5520Info.u32ShutterShortUs;
        pSnrInfo->u32GainShortX1024 = _gPS5520Info.u32GainShortX1024;
        pSnrInfo->u8ResId       = _gPS5520Info.u8ResId;
        pSnrInfo->u8HdrMode     = _gPS5520Info.u8HdrMode;
        pSnrInfo->u32TimeoutMs  = 200;
        //CamOsPrintf("PS5520 , FPS=%u, Shutter=%u, Gain=%u \n", pSnrInfo->u32FpsX1000, pSnrInfo->u32ShutterUs, pSnrInfo->u32GainX1024);
    }
    return 0;
}

/* Sensor EarlyInit implementation end*/
SENSOR_EARLYINIY_ENTRY_IMPL_END( PS5520,
                                 Sensor_init_table,
                                 PS5520EarlyInitShutterAndFps,
                                 PS5520EarlyInitGain,
                                 PS5520EarlyInitGetSensorInfo
                                );
